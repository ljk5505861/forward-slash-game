...
class ArcherBehavior {
  ...
  shoot(target){ const e=this.e,s=this.scene; const fromX=e.x-(e.width||55)*0.35, fromY=e.y-(e.height||86)*0.28; const toX=target.x, toY=target.y-(target.height||60)*0.25; targetDamage(s,target,e,e.damage,{source:'archerArrow',attackType:'projectile',dodgeable:true}); const arrow=s.add?.rectangle?.(fromX,fromY,34,4,0x6b3f1d,0.95)?.setDepth?.(22); if(!arrow) return; arrow.rotation=Phaser.Math.Angle.Between(fromX,fromY,toX,toY); this.arrows.add(arrow); s.tweens?.add?.({targets:arrow,x:toX,y:toY,duration:220,onComplete:()=>{ this.arrows.delete(arrow); arrow.destroy?.(); }}); }
