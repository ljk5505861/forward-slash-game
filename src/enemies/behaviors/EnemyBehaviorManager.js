...
const chooseAnyTarget = (scene, enemy, range=Infinity, options={}) => { const target=scene.combatSystem?.getOrLockEnemyTarget?.(enemy, options) ?? scene.combatSystem?.getSelectableEnemyAttackTargets?.(enemy, options).map(t=>({target:t,distance:Math.hypot(enemy.x-t.x,enemy.y-t.y)})).filter(x=>x.distance<=range).sort((a,b)=>a.distance-b.distance)[0]?.target ?? null; return target&&Math.hypot(enemy.x-target.x,enemy.y-target.y)<=range?target:null; };
