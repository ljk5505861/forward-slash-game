...
export default class ShopPanel {
  constructor(scene){ this.scene=scene; this.nodes=[]; this.itemNodes=[]; this.isOpen=false; this.items=[]; }
  show({items=[]}={}){ if(this.isOpen) return; this.isOpen=true; this.items=items; const s=this.scene; s.beginGameplayPause?.();
    ...
    leave.on('pointerdown',()=>this.hide()); this.render(); }
  ...
  hide(){ if(!this.isOpen) return; this.nodes.concat(this.itemNodes).forEach(n=>{n.removeAllListeners?.();n.destroy();}); this.nodes=[]; this.itemNodes=[]; this.isOpen=false; this.scene.resumeModalFlow?.(); }
  destroy(){ this.hide(); }
}
