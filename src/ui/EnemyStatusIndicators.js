...
export function createEnemyStatusIndicators(scene, enemy){
  ...
  const text=scene.add.text(0,0,'',{fontFamily:'Arial',fontSize:'16px',fontStyle:'bold',color:'#ff9a3d'}).setOrigin(0,0.5);
...
export function updateEnemyStatusIndicators(enemy,burnStacks=0){
  ...
  enemy.burnIndicator.StackText.setPosition(0*STATUS_ITEM_WIDTH,0).setText(stacks>0?String(stacks):'');
