...
export default class Hud {
  constructor(scene){
    ...
    this.statusMessage='';
    this.nodes=[];
    this.leftPanel=scene.add.rectangle(20,24,268,82,0x07101f,0.58).setOrigin(0,0).setScrollFactor(0).setDepth(DEPTH);
    ...
    this.xpBg=scene.add.rectangle(78,80,BAR_W,5,0x151821,0.7).setOrigin(0,0).setScrollFactor(0).setDepth(DEPTH+1);
    this.xpFill=scene.add.rectangle(78,80,0,5,0xffd166,0.75).setOrigin(0,0).setScrollFactor(0).setDepth(DEPTH+2);
    ...
    this.nodes=[this.leftPanel,this.heart,this.hpBg,this.hpFill,this.hpText,this.mpBg,this.mpFill,this.xpBg,this.xpFill,this.rightPanel,this.gold,this.stage,this.settings,this.version,this.boss];
  }
  ...
  update(){
    ...
    this.setBar(this.xpFill,BAR_W,p.xp??0,p.xpToNext??0);
    this.gold.setText(`🪙 ${p.gold??0}`);
    this.stage.setText(this.stageName ? this.stageName.replace(/^阶段\d+：?/,'') : '训练场');
