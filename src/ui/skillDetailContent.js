...
import { ALCHEMY_ID, MATERIAL_MULTIPLIERS, getAlchemyState, getAlchemyRecipe } from '../skills/handlers/CultivationAlchemySkill.js';
...
const FIELD_META=[
  ['damage','伤害','number'],['landingDamage','坠落伤害','number'],['impactDamage','命中伤害','number'],['blastDamage','爆炸伤害','number'],['tickDamage','持续伤害','number'],['splitDamage','分裂伤害','number'],['zoneDamage','区域每次伤害','number'],['burstDamage','爆发伤害','number'],['contactDamage','接触伤害','number'],['burnBurstDamage','燃爆伤害','number'],['biteDamage','撕咬伤害','number'],['burstDamagePerStack','每层爆发伤害','number'],['burnDamage','灼烧每跳伤害','number'],['poisonDamage','中毒每跳伤害','number'],['basePoisonDamage','基础中毒伤害','number'],['flatDamage','固定伤害','number'],
  ...
  ['cooldownMs','冷却时间','seconds'],['riseDurationMs','上升时间','seconds'],['hoverDurationMs','悬浮时间','seconds'],['fallDurationMs','坠落时间','seconds'],['postGravityDurationMs','Lv6落地压制持续时间','seconds'],['devourDurationMs','吞噬时间','seconds'],['warningMs','预警时间','seconds'],['tickIntervalMs','跳伤间隔','seconds'],['pullDurationMs','拉动时间','seconds'],['residualDurationMs','残留区域时间','seconds'],['secondaryImplosionDelayMs','二次坍缩延迟','seconds'],['attackIntervalMs','攻击间隔','seconds'],['intervalMs','攻击/结算间隔','seconds'],['warmupMs','温养时间','seconds'],['durationMs','持续时间','seconds'],['internalCooldownMs','内部冷却','seconds'],['baseCooldownMs','基础生成间隔','seconds'],['minCooldownMs','最短生成间隔','seconds'],['echoDelayMs','残影出手间隔','seconds'],['phaseDurationMs','虚化时间','seconds'],['activeCooldownReduceMs','主动技能减冷却','seconds'],['gainIntervalMs','叠层间隔','seconds'],['graceMs','保留时间','seconds'],['armorBreakMs','破甲持续时间','seconds'],['streakWindowMs','连续闪避窗口','seconds'],['streakReductionMs','每次连闪减冷却','seconds'],['highSpeedIntervalMs','高速积层间隔','seconds'],['eventThrottleMs','同类事件间隔','seconds'],['decayIntervalMs','层数衰减间隔','seconds'],['reviveIntervalMs','重新播种间隔','seconds'],['extendMs','中毒延长时间','seconds'],['poisonMs','中毒持续时间','seconds'],['poisonIntervalMs','中毒跳伤间隔','seconds'],['burnMs','灼烧持续时间','seconds'],['burnIntervalMs','灼烧跳伤间隔','seconds'],['basePoisonMs','基础中毒持续时间','seconds'],['basePoisonIntervalMs','基础中毒间隔','seconds'],['burnBurstCooldownMs','燃爆触发间隔','seconds'],['secondDelayMs','第二把剑延迟','seconds'],['mudShotIntervalMs','泥弹攻击间隔','seconds'],['guardRechargeMs','护体恢复时间','seconds'],['contactCooldownMs','同一敌人接触冷却','seconds'],['postGuardDurationMs','额外减伤持续时间','seconds'],
  ['radius','范围','distance'],['halfWidth','反转半宽','distance'],['riseHeight','升空高度','distance'],['blastRadius','爆炸范围','distance'],['pullDistance','普通怪拉动距离','distance'],['coreHalfWidth','核心半宽','distance'],['outerPullSpeed','外围吸引速度','speed'],['corePullSpeed','核心吸引速度','speed'],['pullDeadZonePx','中心吸引停止距离','distance'],['shockwaveRadiusBonus','震荡额外范围','distance'],['range','射程','distance'],['burstRadius','爆发范围','distance'],['contactKnockback','接触推离强度','distance'],['burnBurstRadius','燃爆范围','distance'],['spreadRadius','传播范围','distance'],['parasiteBurstRadius','寄生爆发范围','distance'],['kingRadiusBonus','毒王额外范围','distance'],['width','宽度','distance'],['projectileSpeed','飞行速度','number'],
  ['shots','发射数量','count'],['guardCharges','护体次数','count'],['strikeCount','下压次数','count'],['orbCount','投射物数量','count'],['swords','飞剑数量','count'],['suns','太阳数量','count'],['pierce','命中目标数','count'],['splitCount','每次分裂数量','count'],['maxSplitGeneration','最大分裂代数','count'],['maxSeedsPerCast','单次最多火种','count'],['maxStacks','最大层数','count'],['poisonStacks','施加中毒层数','count'],['burnStacks','施加灼烧层数','count'],['retainBurnStacksAfterBurst','燃爆后保留层数','count'],['heavyHitEvery','重击触发所需攻击数','count'],['maxAfterimages','最大残影数','count'],['maxCopyAfterimages','参与继承的残影数','count'],['maxLinks','最大连接目标','count'],['maxCount','最大召唤数量','count'],['baseCount','基础召唤数量','count'],['stacksPerExtra','每增加一只所需层数','count'],['maxGrowth','成长上限','number'],['volley','每轮剑数','count'],['spreadTargets','传播目标数','count'],['insectPoisonStacks','毒虫施加中毒层数','count'],['burstStackThreshold','爆发所需感染层数','count'],['burstMaxStacks','爆发计算层数上限','count'],['burstPoisonStacks','爆发附加中毒层数','count'],['attachCount','附身数量上限','countAll'],
  ['damageReduction','伤害减免','percent'],['guardReduction','护体减伤','percent'],['moveSlow','移动减速','percent'],['postMoveSlow','Lv6移动减速','percent'],['postAttackSlow','Lv6攻击减速','percent'],['eliteLiftScale','精英升空比例','percent'],['eliteHoverScale','精英悬浮比例','percent'],['bossDamageScale','Boss伤害比例','percent'],['eliteExecuteThreshold','精英处决阈值','percent'],['eliteDevourDamageRatio','高生命精英额外伤害','percent'],['bossDevourDamageRatio','Boss额外伤害比例','percent'],['attackSlow','攻击速度降低','percent'],['residualMoveSlow','残留移动减速','percent'],['residualAttackSlow','残留攻击减速','percent'],['outerMoveSlow','外围移动减速','percent'],['outerAttackSlow','外围攻击减速','percent'],['coreMoveSlow','核心移动减速','percent'],['coreAttackSlow','核心攻击减速','percent'],['shockwaveDamageRatio','震荡伤害比例','percent'],['suppressedDamageBonus','重力压制增伤','percent'],['followupDamageRatio','后续下压伤害比例','percent'],['sameTargetSecondOrbDamageRatio','同目标第二颗伤害比例','percent'],['secondaryImplosionDamageRatio','二次坍缩伤害比例','percent'],['secondaryImplosionPullRatio','二次坍缩拉动比例','percent'],['resonanceDamageBonus','共振额外伤害','percent'],['dodgeChance','闪避率','percent'],['critChance','暴击率','percent'],['critMultiplierBonus','额外暴击伤害','percent'],['lifeSteal','普通攻击吸血','percent'],['heavyLifeSteal','重击额外吸血','percent'],['heavyHitLifeSteal','重击吸血','percent'],['triggerRatio','触发生命线','percent'],['rearmRatio','重新待命生命线','percent'],['damageBonus','伤害加成','percent'],['heavyDamageBonus','重击伤害加成','percent'],['lifeStealBonus','吸血加成','percent'],['heavyLifeStealBonus','重击吸血加成','percent'],['attackSpeedBonus','攻击速度加成','percent'],['moveSpeedBonus','移动速度加成','percent'],['afterimageDamageBonus','残影伤害加成','percent'],['dodgeBonus','闪避加成','percent'],['physicalVulnerability','目标受到物理伤害增加','percent'],['defensePerStack','每层防御加成','number'],['damageReductionPerStack','每层伤害减免','percent'],['postGuardDamageReduction','Lv3额外减伤','percent'],['emergencyGuardReduction','Lv9临界稳定减伤','percent'],['emergencyProjectedHpRatio','Lv9临界稳定触发生命比例','percent'],['healingReceivedBonus','受到治疗加成','percent'],['absorbRatio','毒伤吸收比例','percent'],['sufferedRatio','本次承伤附加','percent'],['regenRatio','护盾再生比例','percent'],['damageRatio','伤害比例','percent'],['executeRatio','斩杀线','percent'],['copyDamageRatio','继承伤害比例','percent'],['guGrowthContributionRatio','寄生成长贡献比例','percent'],['consumeRatio','感染吞噬比例','percent'],['absorbBonus','寄生吸收加成','percent'],['growthCapBonus','寄生成长上限加成','percent'],['guDamageBonus','寄生伤害加成','percent'],['insectDamageBonus','毒虫伤害加成','percent'],['insectAttackSpeedBonus','毒虫攻速加成','percent'],['insectExtendBonus','毒虫延长加成','percent'],['spreadChance','传播概率','percent'],['kingChanceBonus','毒王传播概率加成','percent'],['healRatio','伤害转治疗比例','percent'],['secondarySunDamageMultiplier','第二太阳伤害比例','percent'],['explosionScale','爆炸伤害比例','percent'],['secondGenerationScale','后续分裂伤害比例','percent'],['lifeStealScale','残影吸血比例','percent'],['playerBonusRatio','玩家伴生属性加成','percent'],['mudShotDamageRatio','泥弹伤害比例','percent'],['summonPowerBonus','召唤物输出/治疗强化','percent'],['summonMaxHpBonus','召唤物最大生命强化','percent'],['summonDamageReduction','召唤物伤害减免','percent'],['summonActionSpeedBonus','召唤物行动速度强化','percent'],['summonHealingReceivedBonus','召唤物受到治疗强化','percent'],
  ['heavyHitMultiplier','重击伤害倍率','multiplier'],['coreDamageMultiplier','核心伤害倍率','multiplier'],['defenseScale','防御系数','multiplier'],['defenseIgnore','防御无视','percent'],['shieldDefenseScale','护盾防御系数','multiplier'],['endShieldMultiplier','结束护盾倍率','multiplier'],['sizeScale','剑体尺寸倍率','multiplier'],['shadowSwordDamageRatio','影剑伤害比例','percent'],['shadowSwordIntervalMultiplier','影剑攻击间隔倍率','multiplier'],['damagePerGrowth','每点成长伤害','number'],
...
const fmt=(type,value)=>{ if(type==='seconds') return secondsText(value); if(type==='speed') return `${numberText(value)}像素/秒`; if(type==='percent') return percentText(value); if(type==='multiplier') return `${numberText(value)}倍`; if(type==='boolean') return value?'是':'否'; if(type==='countAll') return Number(value)>=999?'全部':`${numberText(value)}只`; return numberText(value); };
...
function specialLines(data={}){
  ...
  if(data.elitePullScale!==undefined) lines.push(`精英吸引比例：${percentText(data.elitePullScale)}`);
  if(data.bossPullImmune) lines.push('Boss位置吸引：免疫');
  return lines;
}
...
function alchemyDetail(cfg,level,context={}){
  const scene=context.scene; const st=getAlchemyState(scene)||{}; const recipe=getAlchemyRecipe(scene||context.scene?.skillSystem); const snap=getCultivationSnapshot(scene);
...
export function validateSkillDetailContent(){
  ...
  Object.entries(SKILLS).forEach(([id,cfg])=>{
    if(!cfg.description||(id!=='white_dwarf'&&BLOCKED.some(text=>cfg.description.includes(text)))) problems.push(`${id}:description`);
