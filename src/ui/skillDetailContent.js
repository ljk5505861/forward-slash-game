...
import { SOUL_THRESHOLDS, SWORD_MYTHIC, getSwordFlowState, mainSwordStats, refreshSwordQuality, tombStats, tryPromoteSwordTomb } from '../skills/handlers/SwordFlowState.js';
...
function swordDetail(cfg, level, { scene }={}){
  const sys=scene?.skillSystem; const data=sys?.getData?.('sword_wave',level)||cfg.levels[level-1]; const stats=sys?mainSwordStats(sys,data):{}; const st=stats.state||{}; const nextNeed=SOUL_THRESHOLDS[Math.min(SOUL_THRESHOLDS.length-1,(SOUL_THRESHOLDS.findIndex(x=>(st.effectiveSouls||0)<x) || SOUL_THRESHOLDS.length-1))];
...
function tombDetail(cfg, level, { scene }={}){
  const sys=scene?.skillSystem; if(sys) tryPromoteSwordTomb(sys); const st=sys?getSwordFlowState(sys):{}; const data=sys?.getData?.('sword_tomb',level)||cfg.levels[level-1]; const stats=sys?tombStats(sys,data):{};
