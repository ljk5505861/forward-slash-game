...
export function formatSkillSelectionOption(option={}, playerData={}){
  ...
  const milestoneLines=extractMilestoneChanges(skill,target); const detail=unique([skill.name, data.desc||skill.description, cooldownText(skill,data), ...milestoneLines, `再次点击“${skill.name}”确认`]);
...
export function formatArtifactSelectionOption(option={}){
  if(option.type==='fallback'){
    ...
    return { ...option, kind:'artifact', title:name, subtitle:'属性奖励', rarity:'属性奖励', iconText:name[0], levelText:'', optionLines:['属性奖励'], summaryLines:detailLines.slice(0,2), detailLines:[name,...detailLines,`再次点击“${name}”确认`], category:'属性奖励', confirmText:`再次点击“${name}”确认` };
  }
  ...
  const details=unique([name, effect, ...(option.requiredSkillName?[`需要技能：${option.requiredSkillName}`]:[]), `再次点击“${name}”确认`]);
  return { ...option, kind:'artifact', title:name, subtitle:category, rarity:category, iconText:name[0], iconColor:artifact.color, levelText:'', optionLines:[category], summaryLines:[effect], detailLines:details, category, confirmText:`再次点击“${name}”确认` };
}
...
export function formatProfessionSelectionOption(id){ const p=typeof id==='string'?PROFESSIONS[id]:id||{}; const profile=PROFESSION_ATTACK_PROFILES[p.professionAttackProfile]||{}; const attack={swordSlash:'近战长剑攻击',arcaneBolt:'远程法术弹',hunterArrow:'远程弓箭射击'}[profile.type]||profile.type||'默认攻击'; return { ...p, kind:'profession', title:p.name||p.id||'未知职业', subtitle:'职业', iconText:(p.name||'?')[0], iconColor:p.color, optionLines:['职业'], detailLines:[p.name||p.id||'未知职业',`普攻：${attack}`,`核心机制：${p.mechanic||'暂无'}`,`基础加成：${bonusText(p.bonuses)||'无'}`,`特点：${(p.description||'定位清晰').split('；')[0]}`,`再次点击“${p.name||p.id||'职业'}”确认`], confirmText:`再次点击“${p.name||p.id||'职业'}”确认` }; }
