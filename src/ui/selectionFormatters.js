...
export function formatProfessionSelectionOption(id){ const p=typeof id==='string'?PROFESSIONS[id]:id||{}; const profile=PROFESSION_ATTACK_PROFILES[p.professionAttackProfile]||{}; const attack={swordSlash:'近战长剑攻击',arcaneBolt:'远程法术弹',hunterArrow:'远程弓箭射击'}[profile.type]||profile.type||'默认攻击'; return { ...p, kind:'profession', title:p.name||p.id||'未知职业', subtitle:'职业', iconText:(p.name||'?')[0], iconColor:p.color, optionLines:['职业'], detailLines:[p.name||p.id||'未知职业',`普攻：${attack}`,`核心机制：${p.mechanic||'暂无'}`,`基础加成：${bonusText(p.bonuses)||'无'}`,`特点：${(p.description||'定位清晰').split('；')[0]}`,], confirmText:'' }; }
