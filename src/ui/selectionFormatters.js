...
export class SelectionState { constructor(){this.reset();} reset(){this.selectedIndex=-1;this.selectedOption=null;this.confirmed=false;this.isOpen=false;} open(){this.reset();this.isOpen=true;} select(i,o){ if(!this.isOpen||this.confirmed) return false; this.selectedIndex=i; this.selectedOption=o; return true;} selectOrConfirm(i,o,cb){ if(!this.isOpen||this.confirmed) return 'locked'; if(this.selectedIndex===i){ this.confirmed=true; cb?.(o,i); return 'confirmed'; } this.selectedIndex=i; this.selectedOption=o; return 'selected'; } confirm(cb){ if(!this.isOpen||this.confirmed||this.selectedIndex<0) return false; this.confirmed=true; cb?.(this.selectedOption,this.selectedIndex); return true;} close(){this.reset();} }
