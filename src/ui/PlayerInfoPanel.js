...
export default class PlayerInfoPanel {
  ...
  clearContent(){ this.content.forEach(n=>{n.removeAllListeners?.();n.destroy();}); this.content=[]; this.valueNodes.clear(); this.valueSnapshot.clear(); }
  ...
  refreshValues(force=false){ const next=this.snapshot(); for(const [key,value] of next){ if(!force&&this.valueSnapshot.get(key)===value) continue; this.valueSnapshot.set(key,value); const node=this.valueNodes.get(key); if(node) node.setText(`${node._label}：${value}`); } }
  renderList(title, rows){ this.addContent(this.scene.add.text(DESIGN_WIDTH/2,388,title,{fontFamily:'Arial',fontSize:'28px',color:'#9fd0ff',stroke:'#000',strokeThickness:3}).setOrigin(0.5,0).setScrollFactor(0).setDepth(DEPTH+2)); rows.forEach(([key,label],i)=>{ const col=i%2,row=Math.floor(i/2),x=DESIGN_WIDTH/2-250+col*270,y=442+row*48; const n=this.scene.add.text(x,y,`${label}：`,{fontFamily:'Arial',fontSize:'21px',color:'#f2f6ff',stroke:'#000',strokeThickness:3}).setScrollFactor(0).setDepth(DEPTH+2); n._label=label; this.valueNodes.set(key,n); this.addContent(n); }); }
  renderBuild(){ const n=this.scene.add.text(DESIGN_WIDTH/2-260,392,'',{fontFamily:'Arial',fontSize:'22px',color:'#f2f6ff',stroke:'#000',strokeThickness:3,lineSpacing:10,wordWrap:{width:520,useAdvancedWrap:true}}).setScrollFactor(0).setDepth(DEPTH+2); n._label=''; this.valueNodes.set('buildText',n); this.addContent(n); }
