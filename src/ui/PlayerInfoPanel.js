...
export default class PlayerInfoPanel {
  ...
  render(){ this.content.forEach(n=>n.destroy()); this.content=[]; this.pageText?.setText(`${this.page+1}/2`); const p=this.scene.playerData; if(this.page===0) this.renderList('基础属性', [ ['攻击力',p.attack],['法术强度',p.spellPower??0],['攻击速度',`${Math.round((p.attackSpeedMultiplier||1)*100)}%`],['护甲',p.defense??0],['闪避率',`${Math.round((p.dodgeChance||0)*100)}%`],['最大生命',p.maxHp],['当前生命',p.hp],['最大法力',p.maxMana??0],['当前法力',p.mana??0],['暴击率',`${Math.round((p.critChance||0)*100)}%`],['暴击伤害',`${Math.round((p.critMultiplier||1.5)*100)}%`],['技能急速',`${Math.round((p.skillHaste||0)*100)}%`],['冷却缩减',`${Math.round((p.cooldownReduction||0)*100)}%`],['吸血',`${Math.round((p.lifeSteal||0)*100)}%`],['技能伤害倍率',`${Math.round((p.skillDamageMultiplier||1)*100)}%`],['普攻伤害倍率',`${Math.round((p.normalAttackDamageMultiplier||1)*100)}%`] ]); else this.renderBuild(); }
