...
export default class PlayerInfoPanel {
  ...
  snapshot(){ const p=this.scene.playerData||{}; if(this.page!==0){ const prof=PROFESSIONS[p.professionId]?.name||'未觉醒'; const advanced=ADVANCED_PROFESSIONS[p.advancedProfessionId]?.name||'未进阶'; const arts=(p.artifacts||[]).map(a=>ARTIFACTS[typeof a==='string'?a:a.id]?.name||(typeof a==='string'?a:a.id)).join('、')||'无'; return new Map([['buildText',[`当前阶段：${this.scene.stageSystem?.phase?.()?.name||'-'}`,`当前等级：${p.level}`,`进度等级：${p.level}`,`法力：${p.mana??0}/${p.maxMana??0}`,`体力：${p.stamina??0}/${p.maxStamina??0}`,`二阶职业：${advanced}`,`召唤师契约：${p.summonContractSkillId||'未绑定'}`,`击杀数：${this.scene.killCount}`,`runState：${this.scene.runState}`,`护盾：${p.shield||0}/${p.maxShield||0}`,`战意：${p.battleMarkStacks||0}`,`破军：${this.scene.artifactSystem?.highHpDamageMultiplier?.()>1?'ON':'OFF'}`,`减伤：${pct(Math.max(p.temporaryDamageReduction||0,getEffectiveDamageReduction(p)))}`,`职业：${prof}`,`法宝：${arts}`].join('\n')]]); }
