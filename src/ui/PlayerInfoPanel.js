import { getEffectiveAttack, getTotalStrength, getEffectiveDefense, getEffectiveDamageReduction, getEffectiveCritMultiplier, sumRuntimeBonuses } from '../config/balance.js';
import { DESIGN_WIDTH, DESIGN_HEIGHT } from '../config/gameConfig.js';
import { ARTIFACTS } from '../config/artifacts.js';
import { PROFESSIONS, ADVANCED_PROFESSIONS } from '../config/professions.js';
...
const cap70=v=>Math.max(0,Math.min(0.70,Number(v)||0));
...
export default class PlayerInfoPanel {
  ...
  contractText(p){ const skillId=this.scene.professionSystem?.summonContract?.()||null; if(!skillId) return '未签订'; const slot=(p.skills||[]).findIndex(skill=>skill.id===skillId); const skillName=SKILLS[skillId]?.name||skillId; const contractName=getSummonCreatureContract(skillId)?.name||'召唤契约'; return `${skillName}（槽位${slot+1}）·${contractName}`; }
  snapshot(){ const p=this.scene.playerData||{}; if(this.page!==0){ const prof=PROFESSIONS[p.professionId]?.name||'未觉醒'; const advanced=ADVANCED_PROFESSIONS[p.advancedProfessionId]?.name||'未进阶'; const arts=(p.artifacts||[]).map(a=>ARTIFACTS[typeof a==='string'?a:a.id]?.name||(typeof a==='string'?a:a.id)).join('、')||'无'; return new Map([['buildText',[`当前阶段：${this.scene.stageSystem?.phase?.()?.name||'-'}`,`当前等级：${p.level}`,`进度等级：${p.level}`,`法力：${p.mana??0}/${p.maxMana??0}`,`体力：${p.stamina??0}/${p.maxStamina??0}`,`二阶职业：${advanced}`,`召唤师契约：${this.contractText(p)}`,`击杀数：${this.scene.killCount}`,`runState：${this.scene.runState}`,`护盾：${p.shield||0}/${p.maxShield||0}`,`战意：${p.battleMarkStacks||0}`,`破军：${this.scene.artifactSystem?.highHpDamageMultiplier?.()>1?'ON':'OFF'}`,`减伤：${pct(Math.max(p.temporaryDamageReduction||0,getEffectiveDamageReduction(p)))}`,`职业：${prof}`,`法宝：${arts}`].join('\n')]]); }
    const strength=getTotalStrength(p), base=Number(p.attack)||0, eff=getEffectiveAttack(p);
    return new Map([['attack',`${eff}（基础${base}＋力量${strength}）`],['strength',strength],['spellPower',p.spellPower??0],['attackSpeed',pct((p.attackSpeedMultiplier||1)+bonus(p,'attackSpeedMultiplierBonuses'))],['defense',getEffectiveDefense(p)],['dodge',pct(cap70((p.dodgeChance||0)+bonus(p,'dodgeChanceBonuses')))],['maxHp',p.maxHp??0],['hp',p.hp??0],['maxMana',p.maxMana??0],['mana',p.mana??0],['critChance',pct(cap95((p.critChance||0)+bonus(p,'physicalCritChanceBonuses')))],['critMultiplier',pct(getEffectiveCritMultiplier(p,{physical:true}))],['skillHaste',pct(p.skillHaste||0)],['cooldownReduction',pct(p.cooldownReduction||0)],['lifeSteal',pct((p.lifeSteal||0)+bonus(p,'lifeStealBonuses')+bonus(p,'physicalLifeStealBonuses'))],['skillDamage',pct(p.skillDamageMultiplier||1)],['normalAttackDamage',pct(1+bonus(p,'attackDamageBonuses')+bonus(p,'normalAttackDamageBonuses'))]]); }
