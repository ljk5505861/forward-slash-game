...
export default class UpgradePanel {
  ...
  select(index){ const result=this.state.selectOrConfirm(index,this.options[index],(o)=>this.onConfirm?.(o)); if(result==='locked') return; if(result==='rejected'){ this.updateDebug(); return; } if(result==='confirmed'){ this.cards.forEach(c=>c.nodes.forEach(n=>n.disableInteractive?.())); this.hide(); return; } this.cards.forEach((c,i)=>{ const on=i===index; c.badge?.setVisible(on); c.root.setStrokeStyle(on?6:4,on?0x9fffb6:0x5278c8,on?1:0.55); c.nodes.forEach(n=>n.setAlpha?.(on?1:0.66)); c.nodes.forEach(n=>{ if(n!==c.root) this.scene.tweens.add({targets:n,scale:on?1.05:1,duration:140}); }); c.root.y=c.y+(on?-8:0); }); this.createDetails(this.formatted[index]); this.updateDebug(); }
