...
export default class UpgradePanel {
  ...
  createIconOptions(){ const gap=190, startX=DESIGN_WIDTH/2-gap; this.formatted.forEach((f,i)=>{ const x=startX+i*gap, y=294; const style=rarityStyle(f.rarityId); const frame=this.scene.add.graphics().setScrollFactor(0).setDepth(DEPTH+1); drawRarityFrame(frame,{x,y,width:148,height:148,rarity:f.rarityId,selected:false}); const icon=this.createIcon(x,y-8,f,i,92); const rarity=this.makeText(x,y+70,f.rarity||f.subtitle||'',{fontSize:'16px',color:style.textColor,alpha:0.88,align:'center',wordWrap:{width:130}},[0.5,0]); const group=[frame,...icon,rarity]; this.nodes.push(frame); this.cards[i]={root:frame,nodes:group,x,y,rarityId:f.rarityId}; this.wire(group,i); }); }
  select(index){ const result=this.state.selectOrConfirm(index,this.options[index],(o)=>this.onConfirm?.(o)); if(result==='locked') return; if(result==='rejected'){ this.updateDebug(); return; } if(result==='confirmed'){ this.cards.forEach(c=>c.nodes.forEach(n=>n.disableInteractive?.())); this.hide(); return; } this.cards.forEach((c,i)=>{ const on=i===index; drawRarityFrame(c.root,{x:c.x,y:c.y,width:148,height:148,rarity:c.rarityId,selected:on,alpha:on?1:0.55}); c.nodes.forEach(n=>n.setAlpha?.(on?1:0.52)); c.nodes.forEach(n=>{ if(n!==c.root) this.scene.tweens.add({targets:n,scale:on?1.08:1,duration:140}); }); }); this.createDetails(this.formatted[index]); this.updateDebug(); }
