...
import { MAX_SKILL_SLOTS } from '../systems/SkillSystem.js';
...
export default class UpgradePanel{
  ...
  select(index){ if(!this.state.isOpen||this.state.confirmed) return; if(this.state.selectedIndex===index){ this.state.confirmed=true; const option=this.state.selectedOption; this.cards.forEach(card=>card.nodes.forEach(node=>node.disableInteractive?.())); this.hide(); const result=this.onConfirm?.(option); if(result===false && !this.isOpen){ this.show(this.lastConfig); this.state.select(index,option); this.updateDebug(); } return; } const result=this.state.select(index,this.options[index])?'selected':'locked'; if(result==='locked') return; this.cards.forEach((card,i)=>{ const selected=i===index; drawRarityFrame(card.root,{x:card.x,y:card.y,width:148,height:148,rarity:card.rarityId,selected,alpha:selected?1:0.55}); card.nodes.forEach(node=>node.setAlpha?.(selected?1:0.52)); card.nodes.forEach(node=>{ if(node!==card.root) this.scene.tweens.add({targets:node,scale:selected?1.08:1,duration:140}); }); }); this.createDetails(this.formatted[index]); this.updateDebug(); }
  clearDetails(){
  ...
  showReplacement(option,onReplace,onCancel){
    ...
    (this.scene.playerData.skills||[]).slice(0,MAX_SKILL_SLOTS).forEach((skillData,index)=>{ const config=SKILLS[skillData.id]||{}; const x=140+(index%3)*220,y=438+Math.floor(index/3)*166; const box=makeInteractive(this.scene.add.rectangle(x,y,142,128,0x263f70,0.96).setStrokeStyle(5,0xffd166,1).setScrollFactor(0).setDepth(DEPTH+2)).on('pointerdown',()=>this.onReplace?.(index)); const text=this.scene.add.text(x,y,`${config.name||skillData.id}\nLv.${skillData.level}`,{fontFamily:'Arial',fontSize:'20px',color:'#fff',align:'center',stroke:'#000',strokeThickness:3,wordWrap:{width:124}}).setOrigin(0.5).setScrollFactor(0).setDepth(DEPTH+3); this.nodes.push(box,text); });
  }
