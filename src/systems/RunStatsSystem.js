...
export default class RunStatsSystem {
  ...
  activeMs(){ return this.locked ? this.stats.activePlayTimeMs : (this.scene.getRealActivePlayTime?.() ?? Math.max(0,this.now()-this.stats.runStartedAt-this.pausedDurationMs)); }
  ...
  recordGold(amount,source='unknown'){ this.stats.goldEarned+=(amount||0); this.stats.goldSources[source]=(this.stats.goldSources[source]||0)+(amount||0); }
  recordShopPurchase(item){ this.stats.shopPurchases.push({ id:item.id, price:item.price, stat:item.stat }); }
  ...
  snapshot(){ const active=this.activeMs(); const clock=this.scene.gameSpeed?.snapshot?.()||{}; return { ...this.stats, ...clock, professionName:PROFESSIONS[this.stats.professionId]?.name||this.stats.professionId||'-', activePlayTimeMs:active, msSinceLastUpgrade:active-this.stats.lastUpgradeAt, buildTags:getBuildTags(this.scene.playerData) }; }
}
