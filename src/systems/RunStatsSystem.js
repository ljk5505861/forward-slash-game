...
export default class RunStatsSystem {
  ...
  recordArtifact(p){ const t=this.activeMs(); this.stats.artifactChoices.push(p.artifactId||p.option?.id); if(p.option?.type==='upgrade') this.stats.artifactUpgradesChosen+=1; if(!this.stats.firstArtifactObtainedAt)this.stats.firstArtifactObtainedAt=t; this.stats.artifactLevels={ ...(this.scene.playerData.artifactLevels||{}) }; }
