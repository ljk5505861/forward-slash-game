...
export default class RunStatsSystem {
  ...
  lock(){ this.stats.runEndedAt=this.now(); this.stats.finalHp=this.scene.playerData.hp; this.stats.activePlayTimeMs=this.activeMs(); this.stats.finalSkills=this.scene.playerData.skills.map(s=>({ ...s, name:SKILLS[s.id]?.name||s.id })); this.stats.finalArtifacts=this.scene.playerData.artifacts.map(id=>({ id, name:ARTIFACTS[id]?.name||id, category:ARTIFACTS[id]?.category||'-' })); this.stats.buildTags=getBuildTags(this.scene.playerData); this.locked=true; }
