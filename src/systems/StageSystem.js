...
export default class StageSystem {
  constructor(scene){ this.scene=scene; this.stage=STAGES[0]; this.phaseIndex=0; this.spawnedPhases=new Set(); this.finalBossSpawned=false; this.midBossKilled=false; this.restTriggered=false; this.encounterSequence=0; }
  ...
  update(time){ if(this.scene.isGameplayPaused?.()) return; this.scene.enemyBehaviors?.update(time); this.separateEnemies(); this.scene.enemies.forEach(syncEnemyUi); const p=this.phase(); if(!p) return; if(p.id==='early' && !this.scene.enemies.some(e=>!e.isDefeated)) this.enterPhase(1); else if(p.id==='mixed' && !this.scene.enemies.some(e=>!e.isDefeated)) this.enterPhase(2); else if(p.id==='midBoss' && this.midBossKilled) this.enterPhaseById('late'); else if(p.id==='late' && !this.scene.enemies.some(e=>!e.isDefeated)){ if(this.scene.restCompleted) this.enterPhaseById('finalBoss'); else this.triggerRestPoint(); } }
