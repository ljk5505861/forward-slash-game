...
export default class StageSystem {
  ...
  update(time){ if(this.scene.isGameplayPaused?.()) return; this.scene.enemyBehaviors?.update(time); this.separateEnemies(); this.scene.enemies.forEach(syncEnemyUi); const p=this.phase(); if(!p) return; if(p.id==='early' && !this.scene.enemies.some(e=>!e.isDefeated)) this.enterPhaseById('mixed'); else if(p.id==='mixed' && !this.scene.enemies.some(e=>!e.isDefeated)) this.enterPhaseById('midBoss'); else if(p.id==='midBoss' && this.midBossKilled) return; else if(p.id==='late' && !this.scene.enemies.some(e=>!e.isDefeated)) this.triggerRestPoint(); }
