...
export default class StageSystem {
  ...
  update(time){ if(this.scene.isGameplayPaused?.()) return; this.scene.enemyBehaviors?.update(time); this.separateEnemies(); this.scene.enemies.forEach(syncEnemyUi); const p=this.phase(); if(!p) return; if(p.id==='early' && !this.scene.enemies.some(e=>!e.isDefeated)) this.enterPhase(1); else if(p.id==='mixed' && !this.scene.enemies.some(e=>!e.isDefeated)) this.enterPhase(2); else if(p.id==='midBoss' && this.midBossKilled) this.enterPhase(3); else if(p.id==='late' && !this.scene.enemies.some(e=>!e.isDefeated)){ if(this.scene.restCompleted) this.enterPhase(5); else this.triggerRestPoint(); } }
  onMidBossKilled(){ this.midBossKilled=true; }
  triggerRestPoint(){ if(this.restTriggered) return; this.restTriggered=true; this.phaseIndex=4; this.clearEnemies(); this.scene.eventBus.emit(CombatEvents.REST_REACHED,{ level:this.scene.playerData.level }); this.scene.showRestPoint?.(); }
