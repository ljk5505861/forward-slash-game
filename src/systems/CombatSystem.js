...
export default class CombatSystem {
  ...
  getSelectableEnemyAttackTargets(enemy, options={}){ return this.getAttackableTargets(enemy,options).filter(target=>target.isAlive?.()&&this.isSpiritWolfBlockingPlayer(target)); }
  chooseEnemyAttackTarget(enemy, range=enemy?.attackRange||0, options={}){ const targets=this.getSelectableEnemyAttackTargets(enemy,options); const inRange=targets.map(target=>({ target, distance:Math.hypot((enemy?.x||0)-target.x,(enemy?.y||0)-target.y) })).filter(item=>item.distance<=range).sort((a,b)=>a.distance-b.distance); return inRange[0]?.target||null; }
