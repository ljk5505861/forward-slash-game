...
export default class CombatSystem {
  ...
  reset(){ this.nextPlayerAttackAt=0; this.clearAllKnockbacks(); this.scene.enemies?.forEach(e=>{ e.lockedAttackTarget=null; e.forcePlayerTargetUntilHit=false; }); const p=this.scene.playerData; if(p){ p.heavyHitCounter=0; p.nextAttackIsHeavy=false; p.defenseBonuses={}; p.damageReductionBonuses={}; p.healingReceivedMultiplierBonuses={}; } }
  ...
  damagePlayer(enemy, rawDamage, meta={}){
    if(enemy&&meta.singleTarget===true&&meta.targetType==='player'&&enemy.forcePlayerTargetUntilHit){ enemy.forcePlayerTargetUntilHit=false; enemy.lockedAttackTarget=null; }
    const s=this.scene;
  ...
  getAttackableTargets(enemy, options={}){ const targets=[]; const player=this.getPlayerAttackTarget(); if(options.includePlayer!==false&&player.isAlive()) targets.push(player); const wolves=this.scene.skillSystem?.passiveState?.spiritWolves?.wolves||[]; if(options.includeSpiritWolves!==false) wolves.filter(w=>w?.isAlive?.()).forEach(w=>targets.push(w)); const king=this.getPoisonKingAttackTarget(); if(options.includePoisonKing!==false&&king?.isAlive?.()) targets.push(king); return targets; }
  isSpiritWolfBlockingPlayer(target){ if(target?.type!=='spiritWolf') return true; const playerX=this.scene.player?.x; return !Number.isFinite(playerX) || target.x>=playerX; }
  isValidEnemyAttackTarget(target){ return !!target?.isAlive?.()&&target.isAlive()&&target.active!==false&&!target.isDefeated&&this.isSpiritWolfBlockingPlayer(target); }
  ...
  getOrLockEnemyTarget(enemy, options={}){ if(!enemy) return null; const player=this.getPlayerAttackTarget(); if(enemy.forcePlayerTargetUntilHit&&player.isAlive()){ enemy.lockedAttackTarget=player; return player; } const locked=enemy.lockedAttackTarget; if(locked?.type==='spiritWolf'&&locked.isAlive?.()&&locked.x<(this.scene.player?.x??-Infinity)){ enemy.lockedAttackTarget=null; enemy.forcePlayerTargetUntilHit=true; if(player.isAlive()){ enemy.lockedAttackTarget=player; return player; } } if(this.isValidEnemyAttackTarget(locked)) return locked; enemy.lockedAttackTarget=null; const targets=this.getSelectableEnemyAttackTargets(enemy,options).map(target=>({target,distance:Math.hypot((enemy.x||0)-target.x,(enemy.y||0)-target.y)})).sort((a,b)=>a.distance-b.distance); const target=targets[0]?.target||null; enemy.lockedAttackTarget=target; return target; }
