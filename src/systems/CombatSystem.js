...
export default class CombatSystem {
  ...
  reset(){ this.nextPlayerAttackAt=0; this.clearAllKnockbacks(); this.scene.enemies?.forEach(e=>{ e.lockedAttackTarget=null; }); const p=this.scene.playerData; if(p){ p.heavyHitCounter=0; p.nextAttackIsHeavy=false; p.defenseBonuses={}; p.damageReductionBonuses={}; p.healingReceivedMultiplierBonuses={}; } }
  ...
  calcAttackDamage(weapon, heavyOverride=null, physical=true){ const s=this.scene,p=s.playerData; const heavy=heavyOverride ?? this.consumeHeavyHit(); const factors=this.attackDamageFactors(weapon,heavy); const crit=Math.random()<Math.max(0,Math.min(0.95,(p.critChance||0)+(physical?sumBonuses(p.physicalCritChanceBonuses):0))); const critMult=crit?((p.critMultiplier||1.5)+(physical?sumBonuses(p.physicalCritMultiplierBonuses):0)):1; const baseBeforeProfession=Math.round(factors.nonCritBaseDamage*critMult); return { crit, critResolved:true, heavy, professionMult:factors.professionMult, baseBeforeProfession, nonCritBaseDamage:factors.nonCritBaseDamage, damage:Math.round(baseBeforeProfession*factors.professionMult) }; }
  ...
  damageEnemy(enemy, amount, meta={}){ if(!this.scene.targeting.valid(enemy)) return false; if(!this.scene.targeting.isEnemyFullyInsideViewport(enemy)) return false; const resolved=meta.damageAlreadyResolved===true; const physical=!resolved&&(meta.tags||[]).includes('physical'); let work=resolved?Math.round(amount):(physical?Math.round(amount*(1+sumBonuses(this.scene.playerData?.physicalDamageBonuses))):amount); const critChance=Math.max(0,Math.min(0.95,(this.scene.playerData?.critChance||0)+(meta.bonusCritChance||0)+(physical?sumBonuses(this.scene.playerData?.physicalCritChanceBonuses):0))); const physicalCritBonus=physical?(sumBonuses(this.scene.playerData?.physicalCritChanceBonuses)+sumBonuses(this.scene.playerData?.physicalCritMultiplierBonuses)):0; const canCrit=!resolved&&(!!meta.canCrit||physicalCritBonus>0); const crit=resolved?false:(meta.critResolved?!!meta.crit:(canCrit && Math.random()<critChance)); const critMultiplier=(this.scene.playerData?.critMultiplier||1.5)+(meta.bonusCritMultiplier||0)+(physical?sumBonuses(this.scene.playerData?.physicalCritMultiplierBonuses):0); const critAmount=resolved?work:(meta.critResolved?work:(crit?Math.round(work*critMultiplier):work)); const summonProfessionEligible=(meta.tags||[]).includes(TAGS.SUMMON)&&!meta.noProfessionDamage; const dynamicProfessionMult=this.scene.professionSystem?.getDamageMultiplier?.({ type:meta.source==='skill'?'activeSkill':meta.source, damaging:true, tags:meta.tags||[] },enemy)||1; const professionMult=resolved?(summonProfessionEligible?dynamicProfessionMult:1):(meta.professionApplied&&Number(meta.professionMultiplier)>1 ? meta.professionMultiplier : dynamicProfessionMult); const baseBeforeProfession=Math.max(0, Math.round(meta.baseAmountBeforeProfession ?? (meta.professionApplied && professionMult>0 ? critAmount/professionMult : critAmount))); const boosted=resolved?Math.round(work*professionMult):Math.round(critAmount*professionMult); const physicalTakenBonus=physical?Math.max(0,Math.min(PHYSICAL_DAMAGE_TAKEN_BONUS_CAP,sumBonuses(enemy.physicalDamageTakenBonuses))):0; const physicalTakenMultiplier=1+physicalTakenBonus; const enemyReduction=resolved?0:Math.max(0,Math.min(0.95,enemy.damageReduction||0)); const critIgnore=physical&&crit?sumBonuses(this.scene.playerData?.physicalCritDefenseIgnoreBonuses):0; const explicitIgnore=Number(meta.defenseIgnore||0); const ignore=resolved?0:Math.max(0,Math.min(.9,critIgnore+explicitIgnore)); const effectiveDefense=resolved?0:(enemy.defense||0)*(1-ignore); const reducedBeforeTaken=Math.max(0, Math.round(boosted*(1-enemyReduction)-effectiveDefense)); const baseReducedBeforeTaken=Math.max(0, Math.round(baseBeforeProfession*(1-enemyReduction)-effectiveDefense)); let reduced=resolved?Math.max(0,Math.round(amount)):Math.max(0,Math.round(reducedBeforeTaken*physicalTakenMultiplier)); if(physical&&crit) reduced=Math.round(reduced*(1+sumBonuses(this.scene.playerData?.physicalCritFinalMultiplierBonuses)));  const baseReduced=resolved?reduced:Math.max(0,Math.round(baseReducedBeforeTaken*physicalTakenMultiplier)); const hpBefore=enemy.hp; enemy.hp=Math.max(0, enemy.hp-reduced); const actualDamage=hpBefore-enemy.hp; const actualWithoutProfession=Math.min(hpBefore,baseReduced); const professionBonus=professionMult>1 ? Math.min(actualDamage, Math.max(0, actualDamage-actualWithoutProfession)) : 0; if(actualDamage>0){ const damageTextX=(enemy.nameText?.x ?? enemy.x)+((enemy._damageTextOffsetToggle=!enemy._damageTextOffsetToggle)?-4:4); const damageTextY=enemy.nameText?.y ?? enemy.y-100; this.scene.floatText(damageTextX, damageTextY, `-${actualDamage}`, meta.damageTextColor||(meta.source==='burn'?'#ff8a33':meta.source==='poison'?'#63ff72':crit?'#ffcc33':'#fff')); syncEnemyUi(enemy); this.scene.eventBus.emit(CombatEvents.ENEMY_HIT,{ enemy, damage:actualDamage, professionBonusDamage:professionBonus, crit, ...meta }); if(crit) this.scene.eventBus.emit(CombatEvents.PLAYER_CRIT,{ enemy, damage:actualDamage, forcedCrit:false, source:meta.source, skillId:meta.skillId }); if(!meta.noKnockback&&(meta.applyKnockback||meta.source==='attack'||meta.source==='normalAttack')) this.applyKnockback(enemy, meta); this.applyLifeSteal(actualDamage, meta); this.scene.professionSystem?.onDamageDealt?.(actualDamage,meta); } else syncEnemyUi(enemy); if(enemy.hp<=0){
