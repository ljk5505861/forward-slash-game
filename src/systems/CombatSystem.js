...
export default class CombatSystem {
  ...
  calcAttackDamage(weapon, profile=null, heavyOverride=null, physical=true){ const s=this.scene,p=s.playerData; const heavy=heavyOverride ?? this.consumeHeavyHit(); const factors=this.attackDamageFactors(weapon,profile,heavy); const crit=Math.random()<Math.max(0,Math.min(0.70,(p.critChance||0)+(physical?sumBonuses(p.physicalCritChanceBonuses):0))); const critMult=crit?((p.critMultiplier||1.5)+(physical?sumBonuses(p.physicalCritMultiplierBonuses):0)):1; const baseBeforeProfession=Math.round(factors.nonCritBaseDamage*critMult); return { crit, critResolved:true, heavy, professionMult:factors.professionMult, baseBeforeProfession, nonCritBaseDamage:factors.nonCritBaseDamage, damage:Math.round(baseBeforeProfession*factors.professionMult) }; }
