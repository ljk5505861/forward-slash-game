...
export default class CombatSystem {
  ...
  updateEnemyAttack(enemy,time){ const s=this.scene; if(!s.targeting.valid(enemy)||enemy.isKnockbackActive||isGravityReversalControlled(enemy)) return; if(BEHAVIOR_ATTACKERS.has(enemy.behavior)) return; if(enemy.isBoss && !enemy.enraged && enemy.hp <= enemy.maxHp/2){ enemy.enraged=true; enemy.attackIntervalMs=enemy.enragedAttackIntervalMs; s.hud?.setStatus('Boss 半血狂暴：攻击速度提升！'); } const target=this.chooseEnemyAttackTarget(enemy,enemy.attackRange); if(!target) return; if(time < enemy.nextAttackAt) return; enemy.nextAttackAt=time+getEnemyAttackDelay(enemy,enemy.attackIntervalMs,time); this.damageAttackTarget(target, enemy.damage, { enemy, source:'enemyMelee', attackType:'melee', dodgeable:true, singleTarget:true }); }
}
