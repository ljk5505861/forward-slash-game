import { getEnemyAttackDelay, isGravityReversalControlled } from './EnemyGravityControl.js';
import { isEnemyFrozen } from './EnemyColdControl.js';
...
export const WARRIOR_LANDING_RECOVERY_MS = 260;
...
export const FRONTLINE_SWITCH_THRESHOLD = 18;
...
export default class CombatSystem {
  ...
  reset(){ this.nextPlayerAttackAt=0; this.clearAllKnockbacks(); this.scene.enemies?.forEach(e=>{ e.lockedAttackTarget=null; }); const p=this.scene.playerData; if(p){ p.heavyHitCounter=0; p.nextAttackIsHeavy=false; ['combat_temporary','combat_runtime'].forEach(key=>{ delete p.defenseBonuses?.[key]; delete p.damageReductionBonuses?.[key]; delete p.healingReceivedMultiplierBonuses?.[key]; }); } }
  ...
  update(time){ const s=this.scene; if(s.isGameplayPaused?.()||s.playerData.hp<=0) return; const enemies=s.targeting.all(); const weapon=getWeapon(s.playerData.weaponId); const range=weapon.attackRange*(1+sumBonuses(s.playerData.attackRangeMultiplierBonuses)); const target=s.targeting.nearestAhead(range); if(target && time>=this.nextPlayerAttackAt){ const interval=this.getPlayerAttackInterval(weapon); this.nextPlayerAttackAt=time+interval; this.prepareHeavyHit(); this.performAttack(target, weapon); } enemies.forEach(e=>this.updateEnemyAttack(e,time)); }
  ...
  getPlayerAttackInterval(weapon){ const p=this.scene.playerData; const multiplier=Math.max(0.2,(p.attackSpeedMultiplier||1)+sumBonuses(p.attackSpeedMultiplierBonuses)); const interval=weapon.attackIntervalMs/multiplier; return Math.max(MIN_PLAYER_ATTACK_INTERVAL_MS, interval); }
  performAttack(target, weapon){ return this.performDefaultAttack(target, weapon); }
  attackDamageFactors(weapon, heavy=false){ const s=this.scene,p=s.playerData; const artifactMult=s.artifactSystem?.highHpDamageMultiplier?.() || 1; const professionMult=s.professionSystem?.getDamageMultiplier?.({ type:'normalAttack' })||1; const attackBonus=1+sumBonuses(p.attackDamageBonuses)+(heavy?0:sumBonuses(p.normalAttackDamageBonuses)); const heavyDamageBonus=heavy?1+sumBonuses(p.heavyHitDamageBonuses):1; const heavyMult=heavy?Math.max(1,(p.heavyHitMultiplier||1)*heavyDamageBonus):1; const baseAttack=getEffectiveAttack(p); const nonCritBaseDamage=Math.round(baseAttack*weapon.damageMultiplier*artifactMult*attackBonus*heavyMult); return { professionMult, nonCritBaseDamage }; }
  calcNonCritAttackBaseDamage(weapon, heavyOverride=false){ return this.attackDamageFactors(weapon,!!heavyOverride).nonCritBaseDamage; }
  calcAttackDamage(weapon, heavyOverride=null, physical=true){ const s=this.scene,p=s.playerData; const heavy=heavyOverride ?? this.consumeHeavyHit(); const factors=this.attackDamageFactors(weapon,heavy); const crit=Math.random()<Math.max(0,Math.min(0.95,(p.critChance||0)+(physical?sumBonuses(p.physicalCritChanceBonuses):0))); const critMult=crit?getEffectiveCritMultiplier(p,{physical}):1; const baseBeforeProfession=Math.round(factors.nonCritBaseDamage*critMult); return { crit, critResolved:true, heavy, professionMult:factors.professionMult, baseBeforeProfession, nonCritBaseDamage:factors.nonCritBaseDamage, damage:Math.round(baseBeforeProfession*factors.professionMult) }; }
  applyAttackDamage(enemy, result, tags=['physical'], extra={}){ const s=this.scene; const sourceTags=mergeTags([TAGS.NORMAL_ATTACK], tags, result.heavy?[TAGS.HEAVY_HIT]:[]); const hpBefore=Math.max(0,Number(enemy?.hp)||0); const damaged=this.damageEnemy(enemy,result.damage,{ source:'attack', damageKind:result.heavy?'heavyHit':'normalAttack', heavyHit:result.heavy, tags:sourceTags, canTriggerArtifacts:true, allowLifeSteal:true, professionMultiplier:result.professionMult, baseAmountBeforeProfession:result.baseBeforeProfession, critResolved:true, crit:result.crit, professionApplied:true, ...extra }); const actualDamage=Math.max(0,hpBefore-Math.max(0,Number(enemy?.hp)||0)); if(!damaged) return; s.professionSystem?.onDirectHit?.({ enemy, source:'attack', baseDamage:result.baseBeforeProfession }); if(result.heavy) s.eventBus.emit(CombatEvents.PLAYER_HEAVY_HIT,{ enemy, damage:result.damage, tags:sourceTags }); s.eventBus.emit(CombatEvents.PLAYER_HIT,{ enemy, damage:result.damage, actualDamage, crit:result.crit, heavyHit:result.heavy, forcedCrit:false, tags:sourceTags, source:'attack' }); }
  emitAttackResolved(payload={}){ this.scene.eventBus.emit(CombatEvents.PLAYER_ATTACK_RESOLVED,{ ...payload, source:'attackResolved' }); }
  performDefaultAttack(enemy, weapon){ const s=this.scene; if(!s.targeting.valid(enemy)) return; const heavy=this.consumeHeavyHit(); const baseDamage=this.calcNonCritAttackBaseDamage(weapon,heavy); s.eventBus.emit(CombatEvents.PLAYER_ATTACK,{ enemy, weapon, baseDamage }); this.applyAttackDamage(enemy,this.calcAttackDamage(weapon,heavy),['physical',TAGS.MELEE],{knockback:weapon.knockback}); this.emitAttackResolved({ enemy, targets:[enemy], weapon, baseDamage, profile:null, heavy }); }
  ...
  damageEnemy(enemy, amount, meta={}){ if(!this.scene.targeting.valid(enemy)) return false; if(!this.scene.targeting.isEnemyFullyInsideViewport(enemy)) return false; const resolved=meta.damageAlreadyResolved===true; const physical=!resolved&&(meta.tags||[]).includes('physical'); let work=resolved?Math.round(amount):(physical?Math.round(amount*(1+sumBonuses(this.scene.playerData?.physicalDamageBonuses))):amount); const critChance=Math.max(0,Math.min(0.95,(this.scene.playerData?.critChance||0)+(meta.bonusCritChance||0)+(physical?sumBonuses(this.scene.playerData?.physicalCritChanceBonuses):0))); const physicalCritBonus=physical?(sumBonuses(this.scene.playerData?.physicalCritChanceBonuses)+sumBonuses(this.scene.playerData?.physicalCritMultiplierBonuses)):0; const canCrit=!resolved&&(!!meta.canCrit||physicalCritBonus>0); const crit=resolved?false:(meta.critResolved?!!meta.crit:(canCrit && Math.random()<critChance)); const critMultiplier=getEffectiveCritMultiplier(this.scene.playerData,{physical,bonusCritMultiplier:meta.bonusCritMultiplier}); const critAmount=resolved?work:(meta.critResolved?work:(crit?Math.round(work*critMultiplier):work)); const professionApplied=meta.professionApplied===true; const generalProfessionMult=professionApplied?(Number.isFinite(Number(meta.professionMultiplier))?Number(meta.professionMultiplier):1):(resolved?1:(this.scene.professionSystem?.getGeneralDamageMultiplier?.({ type:meta.source==='skill'?'activeSkill':meta.source, damaging:true, tags:meta.tags||[] })||1)); const targetProfessionMult=this.scene.professionSystem?.getTargetDamageMultiplier?.(enemy)||1; const professionMult=generalProfessionMult*targetProfessionMult; const baseBeforeProfession=Math.max(0, Math.round(meta.baseAmountBeforeProfession ?? (meta.professionApplied && professionMult>0 ? critAmount/professionMult : critAmount))); const boosted=resolved?Math.round(work*targetProfessionMult):(professionApplied?Math.round(critAmount*targetProfessionMult):Math.round(critAmount*professionMult)); const physicalTakenBonus=physical?Math.max(0,Math.min(PHYSICAL_DAMAGE_TAKEN_BONUS_CAP,sumBonuses(enemy.physicalDamageTakenBonuses))):0; const physicalTakenMultiplier=1+physicalTakenBonus; const enemyReduction=resolved?0:Math.max(0,Math.min(0.95,enemy.damageReduction||0)); const critIgnore=physical&&crit?sumBonuses(this.scene.playerData?.physicalCritDefenseIgnoreBonuses):0; const explicitIgnore=Number(meta.defenseIgnore||0); const ignore=resolved?0:Math.max(0,Math.min(.9,critIgnore+explicitIgnore)); const effectiveDefense=resolved?0:(enemy.defense||0)*(1-ignore); const reducedBeforeTaken=Math.max(0, Math.round(boosted*(1-enemyReduction)-effectiveDefense)); const baseReducedBeforeTaken=Math.max(0, Math.round(baseBeforeProfession*(1-enemyReduction)-effectiveDefense)); let reduced=resolved?Math.max(0,Math.round(boosted)):Math.max(0,Math.round(reducedBeforeTaken*physicalTakenMultiplier)); if(physical&&crit) reduced=Math.round(reduced*(1+sumBonuses(this.scene.playerData?.physicalCritFinalMultiplierBonuses)));  const baseReduced=resolved?reduced:Math.max(0,Math.round(baseReducedBeforeTaken*physicalTakenMultiplier)); const hpBefore=enemy.hp, professionImpact={x:enemy.x,y:enemy.y,target:enemy}; enemy.hp=Math.max(0, enemy.hp-reduced); const actualDamage=hpBefore-enemy.hp; const actualWithoutProfession=Math.min(hpBefore,baseReduced); const professionBonus=professionMult>1 ? Math.min(actualDamage, Math.max(0, actualDamage-actualWithoutProfession)) : 0; if(actualDamage>0){ const damageTextX=(enemy.nameText?.x ?? enemy.x)+((enemy._damageTextOffsetToggle=!enemy._damageTextOffsetToggle)?-4:4); const damageTextY=enemy.nameText?.y ?? enemy.y-100; this.scene.floatText(damageTextX, damageTextY, `-${actualDamage}`, meta.damageTextColor||(meta.source==='burn'?'#ff8a33':meta.source==='poison'?'#63ff72':crit?'#ffcc33':'#fff')); syncEnemyUi(enemy); this.scene.eventBus.emit(CombatEvents.ENEMY_HIT,{ enemy, damage:actualDamage, professionBonusDamage:professionBonus, crit, ...meta }); if(crit) this.scene.eventBus.emit(CombatEvents.PLAYER_CRIT,{ enemy, damage:actualDamage, forcedCrit:false, source:meta.source, skillId:meta.skillId }); if(!meta.noKnockback&&(meta.applyKnockback||meta.source==='attack'||meta.source==='normalAttack')) this.applyKnockback(enemy, meta); this.applyLifeSteal(actualDamage, meta); this.scene.professionSystem?.onDamageDealt?.(actualDamage,meta); this.scene.professionSystem?.applySummonSplash?.(professionImpact,actualDamage,meta); } else syncEnemyUi(enemy); if(enemy.hp<=0){
  ...
  clearKnockback(enemy){ if(!enemy) return; enemy.knockbackTween?.stop?.(); enemy.knockbackTween?.remove?.(); enemy.knockbackTween=null; enemy.isKnockbackActive=false; enemy.knockbackUntil=0; enemy.knockbackRecoveryUntil=0; delete enemy.knockbackGroundY; this.knockbackTargets?.delete?.(enemy); }
  ...
  getSpiritBirdAttackTarget(){ const bird=this.scene.spiritBirdRuntime?.getAttackTarget?.(); if(!bird||!bird.isAlive?.()) return null; return bird; }
  ...
  getAttackableTargets(enemy, options={}){ const targets=[]; const player=this.getPlayerAttackTarget(); if(options.includePlayer!==false&&player.isAlive()) targets.push(player); const wolves=this.scene.skillSystem?.passiveState?.spiritWolves?.wolves||[]; if(options.includeSpiritWolves!==false) wolves.filter(w=>w?.isAlive?.()).forEach(w=>targets.push(w)); const king=this.getPoisonKingAttackTarget(); if(options.includePoisonKing!==false&&king?.isAlive?.()) targets.push(king); const bird=this.getSpiritBirdAttackTarget(); if(options.includeSpiritBird!==false&&bird?.isAlive?.()) targets.push(bird); const mantra=this.scene.skillSystem?.passiveState?.mantraHeavenlyBook?.absorb?.ally; if(options.includeMantraAlly!==false&&mantra?.isAlive?.()) targets.push(mantra); return targets; }
  isValidEnemyAttackTarget(target){ return !!target?.isAlive?.()&&target.isAlive()&&target.active!==false&&!target.isDefeated; }
  getSelectableEnemyAttackTargets(enemy, options={}){ return this.getAttackableTargets(enemy,options).filter(target=>this.isValidEnemyAttackTarget(target)); }
  getFrontmostEnemyAttackTarget(enemy, options={}){ return this.getSelectableEnemyAttackTargets(enemy,options).map((target,index)=>({target,index})).sort((a,b)=>(b.target.x-a.target.x)||((Math.hypot((enemy?.x||0)-a.target.x,(enemy?.y||0)-a.target.y))-(Math.hypot((enemy?.x||0)-b.target.x,(enemy?.y||0)-b.target.y)))||(a.index-b.index))[0]?.target||null; }
  getOrLockEnemyTarget(enemy, options={}){ if(!enemy) return null; const candidate=this.getFrontmostEnemyAttackTarget(enemy,options); const locked=enemy.lockedAttackTarget; if(!this.isValidEnemyAttackTarget(locked)){ enemy.lockedAttackTarget=candidate; return candidate; } if(!candidate){ enemy.lockedAttackTarget=null; return null; } if(candidate===locked) return locked; if(candidate.x>locked.x+FRONTLINE_SWITCH_THRESHOLD){ enemy.lockedAttackTarget=candidate; return candidate; } return locked; }
  ...
  updateEnemyAttack(enemy,time){ const s=this.scene; if(!s.targeting.valid(enemy)||enemy.isKnockbackActive||time<(enemy.knockbackRecoveryUntil||0)||isGravityReversalControlled(enemy)||isEnemyFrozen(enemy,time)) { if(isEnemyFrozen(enemy,time)) enemy.body?.setVelocityX?.(0); return; } if(BEHAVIOR_ATTACKERS.has(enemy.behavior)) return; if(enemy.isBoss && !enemy.enraged && enemy.hp <= enemy.maxHp/2){ enemy.enraged=true; enemy.attackIntervalMs=enemy.enragedAttackIntervalMs; s.hud?.setStatus('Boss 半血狂暴：攻击速度提升！'); } const target=this.chooseEnemyAttackTarget(enemy,enemy.attackRange); if(!target) return; if(time < enemy.nextAttackAt) return; enemy.nextAttackAt=time+getEnemyAttackDelay(enemy,enemy.attackIntervalMs,time); this.damageAttackTarget(target, enemy.damage, { enemy, source:'enemyMelee', attackType:'melee', dodgeable:true, singleTarget:true }); }
}
