...
export default class UpgradeSystem {
  ...
  maybeShow(){ const s=this.scene; if(this.panelOpen||this.pending<=0) return; if(s.upgradePanel?.isOpen||s.rewardPanel?.isOpen||s.resultPanel?.isOpen) return; if([RunStates.REWARD,RunStates.VICTORY,RunStates.DEFEAT].includes(s.runState)) return; this.panelOpen=true; s.runState=RunStates.UPGRADING; s.eventBus.emit(CombatEvents.LEVEL_UP,{ level:s.playerData.level }); s.upgradePanel.show('升级三选一', this.rollOptions(), (o)=>this.applyOption(o)); }
