...
export default class ArtifactSystem {
  ...
  heartMirror(payload){ const key=payload.bossType||payload.encounterId||payload.enemy?.id||'boss'; if(this.heartMirrorKeys.has(key)) return; this.heartMirrorKeys.add(key); const amount=20; this.scene.statusEffects?.add(StatusEffects.SHIELD,this.scene.playerData,{ shieldKind:'ordinary', durationMs:600000,value:amount,remainingValue:amount,sourceId:`heart_mirror_${key}` }); this.scene.hud?.update(); }
