...
export default class ArtifactSystem {
  ...
  healOrShield(amount,id){ const s=this.scene; const heal=s.healPlayer?.(amount,'artifact',{artifactId:id})||0; const overflow=amount-heal; if(overflow>0) s.statusEffects?.addPermanentShield(Math.min(8, overflow)); s.floatText(s.player.x,s.player.y-96,heal?`+${heal}`:`盾+${Math.min(8, overflow)}`,'#ff6f9f'); s.hud?.update(); }
