...
export default class ArtifactSystem {
  ...
  // Deprecated compatibility stub: artifact upgrades were removed in v0.8.2. New reward flows must never call this.
  ...
  burn(enemy){ const value=5; this.scene.statusEffects.add(StatusEffects.BURN,enemy,{ durationMs:1800, intervalMs:500, value, sourceId:'flame_heart', maxStacks:3 }); }
  wind(){ if(this.windHits>=4) return; this.windHits+=1; const cd=this.scene.skillSystem.cooldowns.get('spinning_blade'); const reduce=180; if(cd) this.scene.skillSystem.cooldowns.set('spinning_blade',Math.max(this.scene.getGameplayTime()+250,cd-reduce)); }
  battle(){ const p=this.scene.playerData; const max=5; p.battleMarkStacks=Math.min(max,(p.battleMarkStacks||0)+1); this.scene.hud?.update(); }
  update(){}
