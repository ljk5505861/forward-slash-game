...
export default class ArtifactSystem {
  ...
  rollRewardOptions(count=3){ const p=this.scene.playerData; const context=getBuildBiasContext({ skills:p.skills, artifacts:p.artifacts, professionId:p.professionId, config:REWARD_BIAS }); const ownedIds=new Set(p.artifacts.map(artifactIdOf)); const toCandidate=(artifact,type,baseWeight)=>{ const option=this.optionFor(artifact.id,type); const tags=mergeTags(artifact.tags, artifact.supportedTags, artifact.affectedTags, artifact.synergyTags); const bias=calculateBuildBiasWeight({ baseWeight, tags, context }); return { ...option, tags, baseWeight, ...bias }; };
    const freshCandidates=Object.values(ARTIFACTS).filter(a=>!ownedIds.has(a.id)&&this.isValid(a)).map(a=>toCandidate(a,'new',1)); const picked=createWeightedCandidates(freshCandidates,{count,uniqueKey:o=>o.artifactId||o.id}); const upgradeCandidates=p.artifacts.map(artifactIdOf).filter(id=>this.level(id)<2&&this.isValid(ARTIFACTS[id])).map(id=>toCandidate(ARTIFACTS[id],'upgrade',0.85)); if(picked.length<count) createWeightedCandidates(upgradeCandidates,{count:count-picked.length,uniqueKey:o=>o.artifactId||o.id}).forEach(o=>{ if(!picked.some(x=>x.id===o.id)) picked.push(o); }); if(picked.length<count) createWeightedCandidates(FALLBACK_OPTIONS.map(f=>({ ...f, type:'fallback', baseWeight:1, weight:1, tags:[] })),{count:count-picked.length,uniqueKey:o=>o.id}).forEach(o=>{ if(!picked.some(x=>x.id===o.id)) picked.push(o); }); if(this.scene.debugMode) this.scene.lastRewardBiasDebug={ kind:'artifact', enabled:context.enabled, dominantTags:context.dominantTags, candidates:[...freshCandidates,...upgradeCandidates].map(c=>({ id:c.id, title:c.title, tags:c.tags, weight:Number(c.weight.toFixed(3)), baseWeight:c.baseWeight, matchedBuildTags:c.matchedBuildTags, matchedProfessionTags:c.matchedProfessionTags })), picked:picked.map(o=>o.id) }; return picked.slice(0,count); }
