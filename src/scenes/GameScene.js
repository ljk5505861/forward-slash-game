...
export default class GameScene extends Phaser.Scene {
  ...
  canEndGameplayPause(){ return !this.rewardPanel?.isOpen&&!this.upgradePanel?.isOpen&&!this.resultPanel?.isOpen&&!this.pendingArtifactReward&&!(this.upgradeSystem?.pending>0); }
  endGameplayPause(){ if(this.modalPausedAt===null||!this.canEndGameplayPause()) return; if([RunStates.VICTORY,RunStates.DEFEAT].includes(this.runState)) return; const pausedAt=this.modalPausedAt; const pausedDuration=Math.max(0,this.time.now-pausedAt); this.modalPausedAt=null; if(pausedDuration<=0) return; this.combatSystem?.shiftTimers(pausedDuration,pausedAt); this.skillSystem?.shiftTimers(pausedDuration,pausedAt); this.artifactSystem?.shiftTimers(pausedDuration,pausedAt); }
  ...
  cleanupRun(){ this.time.removeAllEvents(); this.tweens.killAll(); this.pendingArtifactReward=null; this.modalPausedAt=null; this.upgradeSystem?.reset(); this.artifactSystem?.cleanup(); this.eventBus?.destroy(); this.upgradePanel?.hide(); this.rewardPanel?.hide(); this.resultPanel?.hide(); this.enemies?.forEach(e=>{ [e.hpBarBg,e.hpBar,e.nameText,e].forEach(o=>o?.destroy()); }); this.enemies=[]; this.skillBar?.destroy(); this.hud?.destroy(); this.debugOverlay?.destroy(); }
