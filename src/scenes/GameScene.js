...
export default class GameScene extends Phaser.Scene {
  ...
  showArtifactReward(){ if(!this.pendingArtifactReward||[RunStates.VICTORY,RunStates.DEFEAT].includes(this.runState)||this.rewardPanel.isOpen) return; this.runState=RunStates.REWARD; const options=Object.values(ARTIFACTS).map(a=>({ id:a.id, title:`${a.name}\n${a.description}`, artifactId:a.id })).slice(0,3); this.rewardPanel.show('法宝奖励三选一', options, (o)=>this.claimArtifactReward(o)); }
  ...
  finishRun(won){ if([RunStates.VICTORY,RunStates.DEFEAT].includes(this.runState)||this.resultPanel?.isOpen) return; this.runState=won?RunStates.VICTORY:RunStates.DEFEAT; this.player?.body?.setVelocityX(0); this.combatSystem?.reset(); this.skillSystem?.reset(); this.pendingArtifactReward=null; this.upgradeSystem?.reset(); this.upgradePanel?.hide(); this.rewardPanel?.hide(); this.eventBus.emit(CombatEvents.RUN_ENDED,{ won }); this.resultPanel.show(won,()=>this.scene.restart()); }
  ...
  cleanupRun(){ this.time.removeAllEvents(); this.tweens.killAll(); this.pendingArtifactReward=null; this.upgradeSystem?.reset(); this.artifactSystem?.cleanup(); this.eventBus?.destroy(); this.upgradePanel?.hide(); this.rewardPanel?.hide(); this.resultPanel?.hide(); this.enemies?.forEach(e=>{ e.burnTick?.remove(false); [e.hpBarBg,e.hpBar,e.nameText,e].forEach(o=>o?.destroy()); }); this.enemies=[]; this.skillBar?.destroy(); this.hud?.destroy(); this.debugOverlay?.destroy(); }
