...
export default class GameScene extends Phaser.Scene {
  ...
  update(time){ if(ACTIVE_STATES.has(this.runState)){ this.movementSystem.update(time); this.combatSystem.update(time); this.skillSystem.update(time); this.artifactSystem.update(time); this.stageSystem.update(time); } else this.player?.body?.setVelocityX(0); this.hud?.update(); this.updateDebugOverlay(); }
  ...
  showArtifactReward(){ if(!this.pendingArtifactReward||[RunStates.VICTORY,RunStates.DEFEAT].includes(this.runState)||this.rewardPanel.isOpen) return; this.beginGameplayPause(); this.runState=RunStates.REWARD; const options=Object.values(ARTIFACTS).map(a=>({ id:a.id, title:`${a.name}\n${a.description}`, artifactId:a.id })).slice(0,3); this.rewardPanel.show('法宝奖励三选一', options, (o)=>this.claimArtifactReward(o)); }
  claimArtifactReward(option){ if(!this.pendingArtifactReward) return; this.artifactSystem.add(option.artifactId); this.pendingArtifactReward=null; this.rewardPanel.hide(); this.resumeModalFlow(); }
  resumeModalFlow(){ if([RunStates.VICTORY,RunStates.DEFEAT].includes(this.runState)) return; if(this.resultPanel?.isOpen) return; if(this.rewardPanel?.isOpen) return; if(this.upgradePanel?.isOpen) return; if(this.pendingArtifactReward){ this.showArtifactReward(); return; } if(this.upgradeSystem?.pending>0){ this.upgradeSystem.maybeShow(); return; } this.endGameplayPause(); this.runState=this.stageSystem?.bossSpawned&&this.enemies?.some(e=>e.isBoss&&!e.isDefeated)?RunStates.BOSS:RunStates.RUNNING; }
  finishRun(won){ if([RunStates.VICTORY,RunStates.DEFEAT].includes(this.runState)||this.resultPanel?.isOpen) return; this.beginGameplayPause(); this.runState=won?RunStates.VICTORY:RunStates.DEFEAT; this.player?.body?.setVelocityX(0); this.combatSystem?.reset(); this.skillSystem?.reset(); this.pendingArtifactReward=null; this.upgradeSystem?.reset(); this.upgradePanel?.hide(); this.rewardPanel?.hide(); this.eventBus.emit(CombatEvents.RUN_ENDED,{ won }); this.resultPanel.show(won,()=>this.scene.restart()); }
  ...
  isGameplayPaused(){ return this.modalPausedAt!==null; }
  beginGameplayPause(){ if(this.modalPausedAt===null) this.modalPausedAt=this.time.now; }
  endGameplayPause(){ if(this.modalPausedAt===null) return; const pausedAt=this.modalPausedAt; const pausedDuration=this.time.now-pausedAt; this.modalPausedAt=null; this.combatSystem?.shiftTimers(pausedDuration,pausedAt); this.skillSystem?.shiftTimers(pausedDuration,pausedAt); this.artifactSystem?.shiftTimers(pausedDuration,pausedAt); }
  updateDebugOverlay(){ if(!this.debugOverlay) return; const now=this.getGameplayTime(); const cds=[...this.skillSystem.cooldowns.entries()].map(([id,t])=>`${id}:${Math.max(0,Math.ceil((t-now)/1000))}`).join(' '); this.debugOverlay.setText([`state:${this.runState}`,`playerX:${Math.round(this.player.x)}`,`target:${this.currentTarget?.name||'-'}`,`enemies:${this.enemies.length}`,`skills:${cds||'-'}`,`stage:${this.stageSystem.stage.name}`]); }
  cleanupRun(){ this.modalPausedAt=null; this.time.removeAllEvents(); this.tweens.killAll(); this.pendingArtifactReward=null; this.upgradeSystem?.reset(); this.artifactSystem?.cleanup(); this.eventBus?.destroy(); this.upgradePanel?.hide(); this.rewardPanel?.hide(); this.resultPanel?.hide(); this.enemies?.forEach(e=>{ this.artifactSystem?.clearBurn(e); [e.hpBarBg,e.hpBar,e.nameText,e].forEach(o=>o?.destroy()); }); this.enemies=[]; this.hud?.destroy(); this.debugOverlay?.destroy(); }
