...
export default class GameScene extends Phaser.Scene {
  ...
  cleanupRun(){ this.time.removeAllEvents(); this.tweens.killAll(); this.pendingArtifactReward=null; this.upgradeSystem?.reset(); this.artifactSystem?.cleanup(); this.eventBus?.destroy(); this.upgradePanel?.hide(); this.rewardPanel?.hide(); this.resultPanel?.hide(); this.enemies?.forEach(e=>{ e.burnTick?.remove(false); [e.hpBarBg,e.hpBar,e.nameText,e].forEach(o=>o?.destroy()); }); this.enemies=[]; this.hud?.destroy(); this.debugOverlay?.destroy(); }
