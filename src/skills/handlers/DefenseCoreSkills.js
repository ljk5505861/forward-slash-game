...
const directSources=new Set(['enemy','enemyMelee','melee','projectile','boss','elite']);
const validDirectDamage = p => !!p?.enemy && directSources.has(p.source||p.attackType||'') && ((p.hpDamage||0)+(p.shieldAbsorbed||0))>0 && !p.blocked;

export const ThornArmorSkill={ bind(system){ let readyAt=0; return system.scene.eventBus.on(CombatEvents.PLAYER_DAMAGED,payload=>{ const s=system.scene,data=system.getData('thorn_armor'),enemy=payload.enemy,now=s.getGameplayTime(); if(!data||!validDirectDamage(payload)||now<readyAt||!s.targeting.valid(enemy)) return; readyAt=now+data.internalCooldownMs; const suffered=(payload.hpDamage||0)+(payload.shieldAbsorbed||0); const base=Math.max(1,Math.round(getEffectiveDefense(s.playerData)*data.defenseScale+data.flatDamage+suffered*(data.sufferedRatio||0))); s.combatSystem.damageEnemy(enemy,base,{ source:'reflect', skillId:'thorn_armor', tags:['physical',TAGS.BUILD_DEFENSE], allowLifeSteal:false, noKnockback:true, defenseIgnore:data.defenseIgnore||0, noThornTrigger:true }); if(data.burstRadius>0){ s.targeting.all().filter(e=>e!==enemy&&Math.hypot(e.x-enemy.x,e.y-enemy.y)<=data.burstRadius).forEach(e=>s.combatSystem.damageEnemy(e,Math.round(base*data.burstRatio),{ source:'reflect', damageKind:'thornBurst', skillId:'thorn_armor', tags:['physical','area',TAGS.BUILD_DEFENSE], allowLifeSteal:false, noKnockback:true, defenseIgnore:data.defenseIgnore||0, noThornTrigger:true })); } s.floatText?.(enemy.x,enemy.y-92,`反伤 ${base}`,'#b7e887'); }); } };

export const GuardianShieldSkill={ bind(system){ const s=system.scene,p=s.playerData; let nextReadyAt=0,lastCombatKey=null,regenDueAt=0,regenCooldownUntil=0; const syncMax=()=>{ p.baseMaxShield??=(p.maxShield||50); p.maxShieldBonuses??={}; const data=system.getData('guardian_shield'); if(data) p.maxShieldBonuses.guardian_shield=data.maxShieldBonus||0; else delete p.maxShieldBonuses.guardian_shield; p.maxShield=(p.baseMaxShield||50)+Object.values(p.maxShieldBonuses||{}).reduce((a,b)=>a+(Number(b)||0),0); s.statusEffects?.syncPlayerDerived?.(); }; const grant=()=>{ const data=system.getData('guardian_shield'); if(!data) return null; syncMax(); const amount=Math.max(1,Math.round(getEffectiveDefense(p)*data.defenseScale+data.flatShield)); const effect=s.statusEffects.add(StatusEffects.SHIELD,p,{ durationMs:data.durationMs??1, persistent:!!data.persistent, expiresNaturally:!data.persistent, value:amount, remainingValue:amount, sourceId:'guardian_shield' }); if(effect){ s.floatText?.(s.player.x,s.player.y-104,`守护盾 +${effect.remainingValue}`,'#8fd7ff'); s.hud?.update?.(); } return effect; }; const updater=()=>{ const data=system.getData('guardian_shield'),now=s.getGameplayTime(); syncMax(); if(!data){ nextReadyAt=0; return; } if(nextReadyAt===0) nextReadyAt=now+data.intervalMs; if(regenDueAt&&now>=regenDueAt){ regenDueAt=0; const amount=Math.round((p.maxShield||0)*(data.regenRatio||0)); if(amount>0) s.statusEffects.add(StatusEffects.SHIELD,p,{ durationMs:1,persistent:true,expiresNaturally:false,value:amount,remainingValue:amount,sourceId:'guardian_shield_regen' }); } if(now>=nextReadyAt){ grant(); nextReadyAt=now+data.intervalMs; } }; const offCombat=s.eventBus.on(CombatEvents.COMBAT_STARTED,payload=>{ const data=system.getData('guardian_shield'),key=`${payload.kind}:${payload.group}:${payload.wave}:${payload.bossId||payload.flowBossType||''}`; if(!data||system.getLevel('guardian_shield')<3||lastCombatKey===key) return; lastCombatKey=key; grant(); nextReadyAt=s.getGameplayTime()+data.intervalMs; }); const offBroken=s.eventBus.on(CombatEvents.SHIELD_BROKEN,()=>{ const data=system.getData('guardian_shield'),now=s.getGameplayTime(); if(!data||system.getLevel('guardian_shield')<9||regenDueAt||now<regenCooldownUntil) return; s.statusEffects?.syncPlayerDerived?.(); if((p.shield||0)>0||(p.permanentShield||0)>0) return; regenDueAt=now+1000; regenCooldownUntil=now+8000; }); system.passiveUpdaters.push(updater); updater(); return ()=>{ offCombat?.(); offBroken?.(); regenDueAt=0; delete p.maxShieldBonuses?.guardian_shield; p.maxShield=(p.baseMaxShield||50)+Object.values(p.maxShieldBonuses||{}).reduce((a,b)=>a+(Number(b)||0),0); s.statusEffects?.syncPlayerDerived?.(); system.passiveUpdaters=system.passiveUpdaters.filter(fn=>fn!==updater); }; } };
