...
export const EntryPoisonNeedleSkill={
  ...
  bind(system){
    ...
    const off=s.eventBus.on(CombatEvents.STATUS_TICK,p=>{
      ...
      const now=s.getGameplayTime(); if(now-windowStart>=1000){ windowStart=now; healedThisWindow=0; pendingPoisonHealing=Math.min(1,Math.max(0,pendingPoisonHealing%1)); }
      ...
      if(finiteCap&&room<1){ pendingPoisonHealing=Math.min(1,Math.max(0,pendingPoisonHealing%1)); return; }
      if(player.hp>=max){ pendingPoisonHealing=Math.min(1,Math.max(0,pendingPoisonHealing%1)); return; }
      pendingPoisonHealing+=p.actualDamage*(data.poisonHealingRatio||0.03);
      const request=Math.floor(pendingPoisonHealing);
      if(request<=0) return;
      const actual=s.healPlayer?.(request,'poison_cloud',{ skillId:'poison_cloud', maxActualHeal:finiteCap?room:undefined })||0;
