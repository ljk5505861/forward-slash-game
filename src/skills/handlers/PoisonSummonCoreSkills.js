...
export const POISON_SUMMON_TUNING = Object.freeze({
  gu:{ baseMaxCount:2, baseHp:48, freeLifeLossPerSecond:4, attachedLifeLossPerSecond:2.5, leechIntervalMs:1000, leechDamage:10, leechHealRatio:1, poisonAbsorbRatio:0.35, poisonEnergyRatio:0.8, splitEnergy:28, reviveMs:3500, transferMs:400, berserkHpRatio:0.3, berserkMs:6000, berserkIntervalMultiplier:0.5, berserkDamageMultiplier:1.5, berserkPoisonEnergyMultiplier:2 },
...
export const ParasiticGuSkill={ bind(system){
  const s=system.scene, guList=[], visuals=new Set(); let nextReviveAt=0, lastAt=s.getGameplayTime(); const runtime={ list:()=>guList.filter(g=>!g.dead), first:()=>guList.find(g=>!g.dead), forceHp:value=>{ const g=guList.find(g=>!g.dead); if(g) g.hp=value; return g; } }; s.parasiticGuRuntime=runtime;
  ...
  const offTick=s.eventBus.on(CombatEvents.STATUS_TICK,p=>{ if(!isNormalPoisonTick(p)) return; const data=system.getData('parasitic_gu'); if(!data) return; guList.filter(g=>!g.dead&&g.host===p.target).forEach(g=>{ const berserk=s.getGameplayTime()<g.berserkUntil; const gain=p.actualDamage*data.poisonAbsorbRatio*(berserk?POISON_SUMMON_TUNING.gu.berserkPoisonEnergyMultiplier:1); healEntity(g,gain,g.maxHp); g.energy+=p.actualDamage*data.poisonEnergyRatio*(berserk?POISON_SUMMON_TUNING.gu.berserkPoisonEnergyMultiplier:1); if(g.energy>=data.splitEnergy) split(g,data); }); });
  const offKill=s.eventBus.on(CombatEvents.ENEMY_KILLED,p=>{ guList.filter(g=>g.host===p.enemy).forEach(g=>attach(g,choose())); });
  const updater=()=>{ const now=s.getGameplayTime(), dt=Math.max(0,now-lastAt)/1000; lastAt=now; const data=system.getData('parasitic_gu'); if(!data){ guList.forEach(kill); return; } if(!guList.some(g=>!g.dead)&&now>=nextReviveAt) spawn(); guList.forEach(g=>{ if(g.dead) return; if(!g.host||!s.targeting.valid(g.host)||!s.statusEffects.has(g.host,StatusEffects.POISON)) attach(g,choose()); g.hp-=data.lifeLossPerSecond*dt; if(g.hp<=0){ kill(g); nextReviveAt=now+data.reviveMs; return; } if(g.hp/g.maxHp>0.55) g.lowHpArmed=true; if(system.getLevel('parasitic_gu')>=9&&g.lowHpArmed&&g.hp/g.maxHp<POISON_SUMMON_TUNING.gu.berserkHpRatio&&now>=g.berserkUntil){ g.berserkUntil=now+POISON_SUMMON_TUNING.gu.berserkMs; g.lowHpArmed=false; }
      const berserk=now<g.berserkUntil; if(g.view){ const target=g.host||s.player; g.view.x+=(target.x-g.view.x)*0.22; g.view.y+=((target.y||s.player.y)-70-g.view.y)*0.22; g.view.setFillStyle(berserk?0xff4b55:0x8ad66f,0.9); g.view.setScale(berserk?1.25:1); }
      if(g.host&&now>=g.nextLeechAt){ const interval=data.leechIntervalMs*(berserk?POISON_SUMMON_TUNING.gu.berserkIntervalMultiplier:1); g.nextLeechAt=now+interval; const dmg=Math.round(data.leechDamage*(berserk?POISON_SUMMON_TUNING.gu.berserkDamageMultiplier:1)); const before=g.host.hp||0; s.combatSystem.damageEnemy(g.host,dmg,{source:'skill',skillId:'parasitic_gu',damageKind:'guLeech',tags:[TAGS.POISON,TAGS.SUMMON,TAGS.BUILD_POISON_SUMMON],allowLifeSteal:false,noKnockback:true,noPoisonChain:true}); healEntity(g,Math.max(0,before-(g.host.hp||0)),g.maxHp); } }); };
  system.passiveUpdaters.push(updater); updater(); return ()=>{ offTick(); offKill(); removeUpdater(system,updater); guList.forEach(kill); visuals.forEach(v=>v.destroy?.()); visuals.clear(); if(s.parasiticGuRuntime===runtime) s.parasiticGuRuntime=null; };
} };
