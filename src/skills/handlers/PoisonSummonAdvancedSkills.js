...
export const PoisonKingSkill={ bind(system){
  const s=system.scene; const p=s.playerData; ensureRuntime(p); const bursted=new WeakSet(); const visuals=new Set(); let appliedLevel=0;
  ...
  const applyBonuses=()=>{ const currentLevel=system.getLevel(SOURCE_POISON_KING); if(currentLevel===appliedLevel) return; ensureRuntime(p); clearBonuses(); appliedLevel=currentLevel; const data=system.getData(SOURCE_POISON_KING,currentLevel); if(!data) return; p.parasiticGuAbsorbBonuses[SOURCE_POISON_KING]=data.absorbBonus; p.parasiticGuGrowthCapBonuses[SOURCE_POISON_KING]=data.growthCapBonus; p.parasiticGuDamageBonuses[SOURCE_POISON_KING]=data.guDamageBonus; p.poisonInsectDamageBonuses[SOURCE_POISON_KING]=data.insectDamageBonus; p.poisonInsectAttackSpeedBonuses[SOURCE_POISON_KING]=data.insectAttackSpeedBonus; p.poisonInsectExtendBonuses[SOURCE_POISON_KING]=data.insectExtendBonus; };
  const addBurstPoison=(target,data)=>{ if(!s.targeting.valid(target)||data.burstPoisonStacks<=0) return; const before=s.statusEffects.getStackCount(target,StatusEffects.POISON); const safeCap=Math.max(before,data.burstPoisonMaxTotalStacks||18); const finalStacks=Math.min(safeCap,before+data.burstPoisonStacks); const addStacks=Math.max(0,finalStacks-before); if(addStacks<=0) return; s.statusEffects.add(StatusEffects.POISON,target,{ durationMs:2400, intervalMs:650, value:4, stacks:addStacks, maxStacks:addStacks, sourceId:`${SOURCE_POISON_KING}_${s.getGameplayTime()}_${target.id||target.name||'target'}`, canSpread:false }); const after=s.statusEffects.getStackCount(target,StatusEffects.POISON); if(after<finalStacks) s.statusEffects.addStacks(target,StatusEffects.POISON,finalStacks-after); };
