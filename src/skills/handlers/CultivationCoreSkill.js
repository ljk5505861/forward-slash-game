...
function ensure(sys){ if(levelOf(sys)<=0) return null; let st=stateOf(sys); if(st) return st; st={realmIndex:0,progress:0,cycleProgressMs:0,lastGameplayAt:sys.scene?.getGameplayTime?.()??0,appliedMaxHpBonus:0,appliedMaxManaBonus:0,breakthroughCount:0,visualTimers:[],visuals:[],visualQueue:[],visualTimer:null,visualTween:null,currentVisuals:[],visualHistory:[],visualPlaying:false,updater:null}; sys.passiveState.ninefoldDao=st; applyStats(sys); return st; }
...
export const NinefoldDaoSkill={ bind(sys){ const st=ensureUpdater(sys); return ()=>{ const current=stateOf(sys); if(current?.updater) sys.passiveUpdaters=sys.passiveUpdaters.filter(fn=>fn!==current.updater); NinefoldDaoSkill.cleanup(sys); }; }, onAcquire(sys){ ensureUpdater(sys); applyStats(sys); }, update(sys){ const st=ensure(sys); if(!st) return; applyStats(sys); const now=sys.scene?.getGameplayTime?.()??0; const elapsed=Math.max(0,now-(st.lastGameplayAt??now)); st.lastGameplayAt=now; if(elapsed<=0) return; if(st.realmIndex<8){ const r=rate(sys,st); if(r>0) addInternal(sys,r*elapsed/1000); if(levelOf(sys)>=3){ st.cycleProgressMs+=elapsed; while(st.realmIndex<8&&st.cycleProgressMs>=30000){ st.cycleProgressMs-=30000; addInternal(sys,r*10); } } } const regen=stat(st.realmIndex).manaRegen*elapsed/1000; if(regen>0) sys.recoverMana?.(regen); }, cleanup(sys){ const st=stateOf(sys), p=sys?.scene?.playerData; if(st&&p){ p.maxHp=Math.max(1,(p.maxHp||0)-(st.appliedMaxHpBonus||0)); p.hp=Math.min(p.hp||0,p.maxHp); p.maxMana=Math.max(0,(p.maxMana||0)-(st.appliedMaxManaBonus||0)); p.mana=Math.min(p.mana||0,p.maxMana); if(p.damageReductionBonuses) delete p.damageReductionBonuses[NINEFOLD_DAO_ID]; } if(st?.updater) sys.passiveUpdaters=sys.passiveUpdaters.filter(fn=>fn!==st.updater); cleanupVisuals(st); if(sys?.passiveState) delete sys.passiveState.ninefoldDao; sys?.scene?.hud?.update?.(); sys?.scene?.skillBar?.update?.(); } };
