...
function startOverload(system,state,data){const now=system.scene.getGameplayTime?.()??0;state.energy=0;state.overloaded=true;state.endsAt=now+data.durationMs;state.extendedMs=0;syncOverloadBonuses(system,state);const level=system.getLevel(OVERLOAD_CORE_ID);if(level>=9){if(system.getLevel(SHOULDER_MISSILE_ID)>0){const missileData=system.getData(SHOULDER_MISSILE_ID);launchShoulderVolley(system,missileData,system.getLevel(SHOULDER_MISSILE_ID),{damageMultiplier:system.scene.playerData.skillDamageMultiplier||1,professionMultiplier:system.scene.professionSystem?.getDamageMultiplier?.({type:'activeSkill',damaging:true})||1,fromMechFreeVolley:true},{free:true});}else{const used=new Set();[0,120,240].forEach(delay=>{const target=(system.scene.targeting?.all?.()||[]).find(e=>!used.has(e));launchEnergyCannon(system,{target,delayMs:delay,excluded:used});if(target)used.add(target);});}}else if(data.ignitionCannon)launchEnergyCannon(system);}
