...
assert.equal(GAME_VERSION,'0.11.10');assert.equal(pkg.version,'0.11.10');
...
{
 const h=harness(9),boss={x:100,y:200,hp:1000,active:true,isBoss:true,body:{}};h.enemies.push(boss);h.scene.playerData.moveSpeedMultiplierBonuses={a:.5,b:.5,c:-.2};for(let i=0;i<8;i++)h.emitAttack();h.tick();assert.equal(h.hits[0].n,Math.round(100*2.15*1.75*1.5));assert.equal(h.knocks.length,0);h.tick(699);for(let i=0;i<8;i++)h.emitAttack();h.tick();assert.equal(h.hits.length,1,'minimum release interval is 700ms');h.tick(1);assert.equal(h.hits.length,2);const runtime=h.system.passiveState.kineticRelease,created=[...h.nodes];h.cleanup();assert.equal(h.system.passiveState.kineticRelease,undefined);assert.equal(h.system.passiveUpdaters.length,0);assert.equal(runtime.visuals.size,0);assert(created.every(n=>n.destroyed));h.emitAttack();assert.equal(getKineticReleaseState(h.system),null);
}
{
