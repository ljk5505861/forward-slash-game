...
const waitScene = makeScene(); const waitStage = new StageSystem(waitScene); waitStage.waveSpawnFinished=true; waitStage.currentWave=1; waitStage.waveState='fighting'; waitStage.updateGroup(0); assert.equal(waitStage.flowState, 'GROUP_COMBAT'); waitStage.updateGroup(800); assert.equal(waitStage.flowState, 'SHOP'); waitStage.onShopClosed('group_1'); assert.equal(waitStage.waveState, 'waitingNextWave'); waitStage.updateGroup(1499); assert.equal(waitStage.currentWave, 1); waitStage.updateGroup(1500); assert.equal(waitStage.currentWave, 2);
