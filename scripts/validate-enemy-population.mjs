...
Object.values(ENEMIES).forEach(cfg => assert.equal(cfg.speed, cfg.kind === 'boss' ? 340 : 300, `${cfg.id} base speed`));
...
const speedScene = makeScene(); const speedEnemy = createEnemy(speedScene, ENEMIES.grunt, 760, BALANCE.groundTopY); assert.equal(speedEnemy.speed, 300); assert.equal('entrySpeed' in speedEnemy, false); assert.equal('combatSpeed' in speedEnemy, false); entryMove(speedScene, speedEnemy); const entryV=speedEnemy.body.vx; approach(speedScene, speedEnemy, speedEnemy.attackRange); assert.equal(Math.abs(entryV), 300); assert.equal(Math.abs(speedEnemy.body.vx), 300, 'entry and active approach use same base speed');
...
const kbCombat = new CombatSystem(overlapScene); a.x=500; b.x=500; c.x=500; kbCombat.applyKnockback(a, {knockback:72}); assert(a.x > 500, 'hit monster is knocked away'); assert.equal(b.x, 500); assert.equal(c.x, 500);
