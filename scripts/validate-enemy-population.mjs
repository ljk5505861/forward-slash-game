...
const makeNode = (x = 0, y = 0, w = 10, h = 10) => ({
  ...
  setStrokeStyle(){ return this; }, setDepth(){ return this; }, setOrigin(){ return this; }, setScrollFactor(){ return this; }, setFillStyle(){ return this; }, setVisible(){ return this; }, setAlpha(){ return this; }, setScale(v){ this.scale=v; return this; }, setDisplaySize(w2,h2){ this.width=w2; this.height=h2; return this; }, setPosition(x2,y2){ this.x=x2; this.y=y2; return this; }, setText(t){ this.text=t; return this; }, destroy(){ this.active=false; },
...
const oldEnemies = { grunt:{width:74,height:118,bodyWidth:64,bodyHeight:108,attackRange:92}, elite:{width:106,height:150,bodyWidth:92,bodyHeight:138,attackRange:105}, armored_guard:{width:98,height:148,bodyWidth:86,bodyHeight:136,attackRange:136}, charger:{width:84,height:106,bodyWidth:76,bodyHeight:96,attackRange:86,chargeTriggerRange:360}, bomber:{width:82,height:118,bodyWidth:72,bodyHeight:106,attackRange:520,preferredRange:350}, healer:{width:78,height:124,bodyWidth:68,bodyHeight:112,attackRange:500,preferredRange:350}, berserker_boss:{width:138,height:184,bodyWidth:122,bodyHeight:170,attackRange:221,chargeSpeed:240}, mid_boss:{width:128,height:178,bodyWidth:114,bodyHeight:166,attackRange:221}, boss:{width:134,height:190,bodyWidth:122,bodyHeight:178,attackRange:221} };
...
assert.equal(ENEMIES.grunt.attackRange, 86, 'grunt melee range tuned for wolf pressure'); assert.equal(ENEMIES.elite.attackRange, 96, 'elite melee range tuned for wolf pressure'); assert.equal(ENEMIES.armored_guard.attackRange, 112, 'armored guard melee range tuned for wolf pressure'); assert.equal(ENEMIES.charger.attackRange, 82, 'charger melee range tuned for wolf pressure'); ['berserker_boss','mid_boss','boss'].forEach(id => assert.equal(ENEMIES[id].attackRange, Math.round(oldEnemies[id].attackRange*0.7), `${id} melee range scaled`));
...
const waitScene = makeScene(); const waitStage = new StageSystem(waitScene); waitStage.waveSpawnFinished=true; waitStage.currentWave=1; waitStage.waveState='fighting'; waitStage.updateGroup(0); assert.equal(waitStage.flowState, 'GROUP_COMBAT'); waitStage.updateGroup(800); assert.equal(waitStage.flowState, 'GROUP_COMBAT'); assert.equal(waitStage.waveState, 'waitingNextWave'); waitStage.updateGroup(2299); assert.equal(waitStage.currentWave, 1); waitStage.updateGroup(2300); assert.equal(waitStage.currentWave, 2);
