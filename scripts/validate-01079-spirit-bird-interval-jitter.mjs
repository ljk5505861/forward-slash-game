...
{ const s=scene(); const bird=addLevels(s,1); s.playerData.hp=400; tick(s,6000); assert.equal(s.playerData.hp,410,'actual Lv1 heal amount is applied once'); const heals=s.floatTexts.filter(f=>f.color==='#7dff8a'); assert.equal(heals.length,1,'one green heal number is shown for one actual heal'); assert.deepEqual(heals[0],{x:s.player.x,y:s.player.y-100,text:'+10',color:'#7dff8a'}); assert.equal(s.floatTexts.filter(f=>f.text==='+10'&&f.color==='#7dff8a').length,1,'does not duplicate the same heal number'); assert.equal(bird.nextHealAt,12000,'next Lv1 heal is scheduled 6000ms later'); }
...
let s=scene(); addLevels(s,9); const wolf={type:'spiritWolf',x:1,y:1,hp:50,maxHp:100,isAlive(){return this.hp>0},heal(a){const b=this.hp; this.hp=Math.min(this.maxHp,this.hp+a); return this.hp-b}}; s.skillSystem.passiveState.spiritWolves={wolves:[wolf]}; s.playerData.hp=400; tick(s,4199); assert.equal(wolf.hp,50); tick(s,1); assert.equal(s.playerData.hp,426); assert.equal(wolf.hp,76);

s=scene(); let bird=addLevels(s,1); s.playerData.hp=400; tick(s,3000); addLevels(s,1); assert.equal(s.playerData.hp,400,'upgrade does not heal immediately'); tick(s,2999); assert.equal(s.playerData.hp,400,'upgrade does not reset current cycle'); tick(s,1); assert.equal(s.playerData.hp,411,'current cycle completes using original due time'); s.playerData.hp=400; tick(s,5799); assert.equal(s.playerData.hp,400); tick(s,1); assert.equal(s.playerData.hp,411,'next cycle uses Lv2 interval');

s=scene(); bird=addLevels(s,1); s.playerData.hp=400; tick(s,1000); const pauseAt=s.now; s.now+=10000; s.skillSystem.shiftTimers(10000,pauseAt); s.skillSystem.update(s.now); assert.equal(s.playerData.hp,400); tick(s,4999); assert.equal(s.playerData.hp,400); tick(s,1); assert.equal(s.playerData.hp,410);
...
s=scene(); bird=addLevels(s,1); const stableY=bird.y, stableVisual=bird.visualY, viewY=bird.view.y, barY=bird.hpBar.y; for(let i=0;i<24;i++){ s.player.y += i%2===0 ? 0.4 : -0.4; tick(s,16); assert.equal(bird.y,stableY); assert.equal(bird.visualY,stableVisual); assert.equal(bird.view.y,viewY); assert.equal(bird.hpBar.y,barY); assert(Number.isInteger(bird.view.x)); assert(Number.isInteger(bird.view.y)); assert(Number.isInteger(bird.hpBar.x)); assert(Number.isInteger(bird.hpBar.y)); }
const bx=bird.x; s.player.x+=100; tick(s,16); assert.equal(bird.x,s.player.x-70); s.player.x-=160; tick(s,16); assert(bird.x>=bx); s.player.x=bird.x+60; const hold=bird.x; tick(s,16); assert.equal(bird.x,hold); s.player.x=bird.x+90; tick(s,16); assert.equal(bird.x,s.player.x-70);
