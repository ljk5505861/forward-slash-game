...
{ const t=target(300); const {scene,calls,arrows}=makeScene([t]); const e=enemy(700); const m=new EnemyBehaviorManager(scene); m.attach(e); m.update(0); assert.equal(e.body.vx,0,'archer inside 450 range stops'); assert.equal(calls.length,1,'one arrow attack deals damage once'); assert.equal(calls[0].meta.source,'archerArrow'); assert.equal(calls[0].meta.attackType,'projectile'); assert.equal(calls[0].meta.dodgeable,true); assert.equal(arrows.length,1,'one arrow visual is spawned'); scene.tweens.added[0].onComplete(); assert.equal(arrows[0].destroyed,true,'arrow visual is destroyed after tween'); }
