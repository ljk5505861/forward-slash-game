...
assert.equal(JSON.parse(fs.readFileSync('package.json','utf8')).version,'0.11.8');
...
{ const mythics=Object.values(SKILLS).filter(s=>s.rarity==='MYTHIC'||s.ultimateSkill); assert.equal(mythics.length,10); assert(mythics.some(s=>s.id==='lightning_tribulation')); assert.equal(Object.values(SKILLS).filter(s=>s.tags?.includes(TAGS.BUILD_WEAPON)).length,3); ['lightning_enchant','lightning_mark','lightning_tribulation'].forEach(id=>assert(SKILLS[id])); }
