...
assert.equal(GAME_VERSION, '0.10.1');
...
const replaceScene=makeScene(); replaceScene.stageSystem=new StageSystem(replaceScene); replaceScene.stageSystem.flowState=LevelFlowStates.SKILL_REWARD; replaceScene.stageSystem.awaitingSkillRewardClose=true; replaceScene.upgradeSystem.pending=1; replaceScene.playerData.skills=[{id:'fireball',level:1},{id:'mirror_march',level:1},{id:'shadow_fist',level:1},{id:'poison_cloud',level:1}]; replaceScene.skillSystem.cooldowns.set('mirror_march',123); const clone={destroyed:false,destroy(){this.destroyed=true;}}; replaceScene.skillSystem.passiveState.mirrorClones=[clone]; replaceScene.skillSystem.active=[{skillId:'mirror_march',ended:false,onEnd(){this.ended=true;}},{skillId:'fireball'}];
...
assert.equal(replaceScene.upgradeSystem.confirmReplacement(1), true); assert.equal(replaceScene.playerData.skills[1].id,'sword_wave'); assert.equal(replaceScene.playerData.skills[1].level,1); assert.deepEqual(replaceScene.playerData.skills.map(s=>s.id), ['fireball','sword_wave','shadow_fist','poison_cloud']); assert(!replaceScene.skillSystem.cooldowns.has('mirror_march')); assert.equal(clone.destroyed,true); assert.equal(replaceScene.skillSystem.active.some(a=>a.skillId==='mirror_march'),false); assert.equal(replaceScene.playerData.upgradesChosen.length,1); assert.equal(replaceScene.eventBus.events.filter(e=>e.type==='UPGRADE_CHOSEN').length,1); assert.equal(replaceScene.upgradeSystem.pending,0); assert.equal(replaceScene.stageSystem.groupIndex,1); assert.equal(replaceScene.upgradeSystem.confirmReplacement(1), false); assert.equal(replaceScene.playerData.upgradesChosen.length,1);

const cancelScene=makeScene(); cancelScene.playerData.skills=[{id:'fireball',level:1},{id:'lightning',level:1},{id:'shadow_fist',level:1},{id:'poison_cloud',level:1}]; cancelScene.stageSystem=new StageSystem(cancelScene); cancelScene.stageSystem.flowState=LevelFlowStates.SKILL_REWARD; cancelScene.stageSystem.awaitingSkillRewardClose=true; cancelScene.upgradeSystem.pending=1; const cancelOptions=[{type:'newSkill',id:'new_sword_wave',skillId:'sword_wave'}]; cancelScene.upgradeSystem.applyOption(cancelOptions[0],{source:RewardSources.STAGE_SKILL_REWARD,originalOptions:cancelOptions}); cancelScene.upgradeSystem.cancelReplacement(); assert.deepEqual(cancelScene.playerData.skills.map(s=>s.id), ['fireball','lightning','shadow_fist','poison_cloud']); assert.equal(cancelScene.playerData.upgradesChosen.length,0); assert.equal(cancelScene.eventBus.events.filter(e=>e.type==='UPGRADE_CHOSEN').length,0); assert.equal(cancelScene.upgradeSystem.pending,1); assert.equal(cancelScene.stageSystem.groupIndex,0); assert.deepEqual(cancelScene.upgradePanel.lastConfig.options, cancelOptions);

for (const source of [RewardSources.CAMPFIRE, RewardSources.DEBUG, RewardSources.OTHER_FREE_REWARD]) { const freeScene=makeScene(); freeScene.stageSystem=new StageSystem(freeScene); freeScene.stageSystem.flowState=LevelFlowStates.SKILL_REWARD; freeScene.stageSystem.awaitingSkillRewardClose=true; freeScene.upgradeSystem.pending=1; freeScene.playerData.skills=[{id:'fireball',level:1}]; freeScene.upgradeSystem.applyOption({type:'skillLevel',id:`lv_fireball_${source}`,skillId:'fireball'},{free:true,source}); assert.equal(freeScene.skillSystem.getLevel('fireball'),2); assert.equal(freeScene.stageSystem.groupIndex,0); assert.equal(freeScene.upgradeSystem.pending,1); }
