...
const scene={ player:{ x: 100 }, balance:{ enemies:{} } };
const entryMove = (_scene, enemy) => enemy.body.setVelocityX(-(enemy.speed||40));
const approach = (scene, enemy, min=enemy.attackRange) => { const dx=scene.player.x-enemy.x; const d=Math.abs(dx); const sp=enemy.speed||40; enemy.body.setVelocityX(d>min+8 ? Math.sign(dx||1)*sp : 0); };
