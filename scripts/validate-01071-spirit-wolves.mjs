...
{
  const {s,sys,state}=makeSystemWithSkill(1); update(sys,s,0); const w=state().wolves[0]; w.defense=0; w.takeDamage(1,{enemy:{x:w.x+20,isElite:false},attackType:'melee'}); assert.equal(w.pendingKnockbackDistance,34,'normal enemy default wolf knockback queues 34px'); w.pendingKnockbackDistance=0; w.takeDamage(1,{enemy:{x:w.x+20,isElite:true},attackType:'melee'}); assert.equal(w.pendingKnockbackDistance,18,'elite default wolf knockback queues 18px'); w.pendingKnockbackDistance=0; w.takeDamage(1,{enemy:{x:w.x+20,isBoss:true},attackType:'melee'}); assert.equal(w.pendingKnockbackDistance,10,'boss default wolf knockback queues 10px');
}
{
  const {s,sys,state}=makeSystemWithSkill(1); update(sys,s,0); const w=state().wolves[0]; w.defense=0; let x=w.x; w.takeDamage(1,{enemy:{x:w.x+20,isElite:false},attackType:'projectile',knockbackDistance:0}); assert.equal(w.x,x,'custom 0px wolf knockback does not move'); x=w.x; w.takeDamage(1,{enemy:{x:w.x+20,isElite:false},attackType:'projectile',knockbackDistance:-10}); assert.equal(w.x,x,'negative custom wolf knockback clamps to 0 and does not push forward'); x=w.x; w.takeDamage(1,{enemy:{x:w.x+20,isElite:false},attackType:'projectile',knockbackDistance:'bad'}); assert.equal(w.pendingKnockbackDistance,34,'non-numeric custom wolf knockback falls back to normal default');
}
{
...
console.log('v0.10.75 spirit wolves validation passed.');
