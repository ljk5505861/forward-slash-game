...
function enemy(x=220,y=100,hp=500){ return {x,y,hp,maxHp:hp,active:true,isDefeated:false,defense:0}; }
...
{
  const {s,sys,state}=makeSystemWithSkill(1); s.playerData.baseAttack=100; s.playerData.attack=999; s.playerData.physicalDamageBonuses.last_stand=9; update(sys,s,0); update(sys,s,8000); const w=state().wolves[0]; const armored=enemy(w.x+40,w.y,500); armored.defense=5; s.enemies.push(armored); update(sys,s,8900); const hit=s.calls.damage.find(d=>d.meta.damageKind==='summonMelee'); assert.equal(hit.amount,15,'summon attack uses inherited attack minus enemy defense only'); assert.equal(hit.meta.tags.includes('physical'),false,'summon attack bypasses player physical bonus pipeline'); assert.equal(hit.meta.canTriggerArtifacts,false);
}
{
