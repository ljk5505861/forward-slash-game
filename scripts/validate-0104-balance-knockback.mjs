...
import CombatSystem, { MIN_PLAYER_ATTACK_INTERVAL_MS, NORMAL_ATTACK_KNOCKBACK_DURATION_MS, NORMAL_ATTACK_KNOCKBACK_LIFT_PX, NORMAL_ATTACK_KNOCKBACK_BOUNCE_MS, NORMAL_ATTACK_KNOCKBACK_BOUNCE_LIFT_PX } from '../src/systems/CombatSystem.js';
...
assert.equal(WEAPONS.short_sword.attackRange, 115, 'normal attack range unchanged');
...
assert.equal(NORMAL_ATTACK_KNOCKBACK_DURATION_MS, 440, 'normal attack knockback duration is 440ms');
assert.equal(NORMAL_ATTACK_KNOCKBACK_LIFT_PX, 24, 'normal attack knockback lift remains 24px');
assert.equal(NORMAL_ATTACK_KNOCKBACK_BOUNCE_MS, 140, 'landing bounce duration is 140ms');
assert.equal(NORMAL_ATTACK_KNOCKBACK_BOUNCE_LIFT_PX, 8, 'landing bounce lift is 8px');
assert.equal(BALANCE.stageWorldWidth, 50000, 'stage world width remains 50000');
...
scene.playerData.attackSpeedMultiplier=1;
const completeMain = enemy => { scene.tweens.last.targets.t=1; scene.tweens.last.onComplete(); return enemy; };
const complete = enemy => { completeMain(enemy); assert.equal(scene.tweens.last.duration, NORMAL_ATTACK_KNOCKBACK_BOUNCE_MS, 'knockback enters landing bounce'); scene.tweens.last.targets.t=1; scene.tweens.last.onComplete(); return enemy; };

const right=makeEnemy({x:500}); right.body.owner=right; combat.applyKnockback(right,{knockback:WEAPONS.short_sword.knockback}); assert.equal(right.isKnockbackActive,true); assert.equal(scene.tweens.last.duration,440, 'arced knockback duration is doubled to 440ms'); assert.equal(right.x,500,'does not jump instantly'); scene.tweens.last.targets.t=0.5; scene.tweens.last.onUpdate(); assert(right.x>500 && right.x<572); assert(Math.abs(right.y-(800-24))<=1, 'halfway through knockback reaches peak lift'); completeMain(right); assert.equal(right.x,572); assert.equal(right.y,800); assert.equal(right.isKnockbackActive,true, 'landing bounce remains knockback active'); assert.equal(scene.tweens.last.duration,140); scene.tweens.last.targets.t=0.5; scene.tweens.last.onUpdate(); assert.equal(right.x,572, 'landing bounce has no horizontal movement'); assert(Math.abs(right.y-(800-8))<=1, 'landing bounce reaches 8px lift'); scene.tweens.last.targets.t=1; scene.tweens.last.onComplete(); assert.equal(right.x,572); assert.equal(right.y,800); assert.equal(right.isKnockbackActive,false); assert(right.hpBarBg.x===right.x && right.nameText.x===right.x);
...
const orderScene=makeScene();
orderScene.player={x:0,y:800};
orderScene.playerData={...orderScene.playerData, hp:100, attack:10, critChance:0, critMultiplier:1.5, attackSpeedMultiplier:1, damageReduction:0, defense:0, weaponId:'short_sword'};
const orderEnemy=makeEnemy({x:WEAPONS.short_sword.attackRange,y:800,nextAttackAt:0,attackRange:999,hp:20}); orderEnemy.body.owner=orderEnemy;
orderScene.enemies=[orderEnemy];
orderScene.targeting={valid:e=>!!e&&!e.isDefeated&&e.active,isEnemyFullyInsideViewport(){return true;},all(){return orderScene.enemies.filter(e=>e.active&&!e.isDefeated);},nearestAhead(range){return this.all().filter(e=>e.x>=orderScene.player.x&&e.x-orderScene.player.x<=range).sort((a,b)=>a.x-b.x)[0]||null;}};
const orderCombat=new CombatSystem(orderScene); const order=[]; orderScene.eventBus.emit=(type)=>{ if(type==='PLAYER_ATTACK') order.push('player'); }; orderCombat.damagePlayer=()=>order.push('enemy');
orderCombat.update(1000); assert.deepEqual(order,['player'], 'player normal attack resolves before enemy melee in the same frame'); assert.equal(orderEnemy.isKnockbackActive,true, 'same-frame hit enemy is knocked back before it can attack'); assert.equal(orderCombat.nextPlayerAttackAt,1333, 'short sword attack interval remains 333ms'); assert.equal(orderEnemy.x,WEAPONS.short_sword.attackRange, 'knockback tween does not teleport immediately'); assert.equal(orderEnemy.knockbackUntil,1580, 'knockbackUntil covers the full 580ms control flow');
const cooldownEnemy=makeEnemy({x:60,y:800,nextAttackAt:0,attackRange:999,hp:20}); cooldownEnemy.body.owner=cooldownEnemy; orderScene.enemies=[cooldownEnemy]; order.length=0; orderCombat.nextPlayerAttackAt=9999; orderCombat.update(2000); assert.deepEqual(order,['enemy'], 'enemy can attack normally when player attack cooldown is not ready');
...
scene.player.x=100; const pauseEnemy=makeEnemy({x:500}); pauseEnemy.body.owner=pauseEnemy; combat.applyKnockback(pauseEnemy,{knockback:72}); scene.tweens.last.targets.t=0.25; scene.tweens.last.onUpdate(); const pausedX=pauseEnemy.x, pausedY=pauseEnemy.y; combat.pauseKnockbacks(); const activeTween=scene.tweens.items.at(-1); assert.equal(activeTween.paused,true); for(let i=0;i<5;i+=1){ if(!activeTween.paused){ scene.tweens.last.targets.t+=0.2; scene.tweens.last.onUpdate(); } } assert.equal(pauseEnemy.x,pausedX); assert.equal(pauseEnemy.y,pausedY); combat.resumeKnockbacks(); assert.equal(activeTween.paused,false); scene.tweens.last.targets.t=0.5; scene.tweens.last.onUpdate(); assert(pauseEnemy.x>pausedX && pauseEnemy.x<572, 'resume continues from paused progress'); scene.tweens.last.targets.t=1; scene.tweens.last.onComplete(); scene.tweens.last.targets.t=1; scene.tweens.last.onComplete(); assert.equal(pauseEnemy.x,572);
...
const repeat=makeEnemy({x:500}); repeat.body.owner=repeat; combat.applyKnockback(repeat,{knockback:72}); const firstTween=repeat.knockbackTween; scene.tweens.last.targets.t=0.4; scene.tweens.last.onUpdate(); const repeatY=repeat.y; combat.applyKnockback(repeat,{knockback:72}); assert.equal(firstTween.stopped,true, 'repeated knockback clears previous tween'); assert.equal(firstTween.removed,true, 'repeated knockback removes previous tween'); assert.equal(repeat.isKnockbackActive,true); assert.notEqual(repeat.knockbackTween,firstTween, 'repeated knockback creates one replacement tween'); assert.equal(scene.tweens.last.duration,440, 'replacement knockback duration is still 440ms'); assert.equal(combat.knockbackTargets.size,1, 'only one active knockback target after replacement'); assert.equal(repeat.knockbackGroundY,800, 'repeated knockback keeps original ground Y'); assert(repeatY<800, 'second knockback can start while target is airborne'); const repeatStartX=repeat.x, repeatStartY=repeat.y; scene.tweens.last.targets.t=0; scene.tweens.last.onUpdate(); assert.equal(repeat.x,repeatStartX, 'replacement knockback starts from current x'); assert.equal(repeat.y,repeatStartY, 'replacement knockback starts from current y without snap'); scene.tweens.last.targets.t=0.5; scene.tweens.last.onUpdate(); assert(repeat.x>repeatStartX, 'replacement knockback continues away from player'); scene.tweens.last.targets.t=1; scene.tweens.last.onComplete(); assert.equal(repeat.isKnockbackActive,true, 'repeated knockback remains controlled during bounce'); const bounceTween=repeat.knockbackBounceTween; combat.applyKnockback(repeat,{knockback:72}); assert.equal(bounceTween.stopped,true, 'repeated knockback during bounce clears bounce tween'); assert.equal(combat.knockbackTargets.size,1); scene.tweens.last.targets.t=1; scene.tweens.last.onComplete(); scene.tweens.last.targets.t=1; scene.tweens.last.onComplete(); assert.equal(repeat.y,800, 'repeated knockback lands on original ground Y');
const chain=makeEnemy({x:500}); chain.body.owner=chain; combat.applyKnockback(chain,{knockback:72}); for(let i=0;i<2;i+=1){ scene.tweens.last.targets.t=333/440; scene.tweens.last.onUpdate(); combat.applyKnockback(chain,{knockback:72}); assert.equal(combat.knockbackTargets.size,1, 'three-hit chain keeps one active knockback target'); } scene.tweens.last.targets.t=1; scene.tweens.last.onComplete(); scene.tweens.last.targets.t=1; scene.tweens.last.onComplete(); assert.equal(chain.y,800, 'three-hit chain lands on initial ground Y');
...
console.log('[validate-0104-balance-knockback] PASS 0.10.5 balance, cooldowns, directional arced knockback, pause compatibility, and attack blocking');
