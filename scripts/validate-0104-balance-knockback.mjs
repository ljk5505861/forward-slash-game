...
import CombatSystem, { MIN_PLAYER_ATTACK_INTERVAL_MS, NORMAL_ATTACK_KNOCKBACK_DURATION_MS, NORMAL_ATTACK_KNOCKBACK_LIFT_PX } from '../src/systems/CombatSystem.js';
...
const player={x:100,y:200,width:BALANCE.player.width,height:BALANCE.player.height,displayWidth:90,displayHeight:156,body:{width:BALANCE.player.bodyWidth,height:BALANCE.player.bodyHeight}};
...
assert.equal(playerHealthBarY(player),player.y-BALANCE.player.bodyHeight/2-10,'health bar uses stable body height');
player.displayWidth=180; player.displayHeight=312;
assert.deepEqual(playerHealthBarSize(player),size,'attack animation display scaling does not resize health bar');
assert.equal(playerHealthBarY(player),player.y-BALANCE.player.bodyHeight/2-10,'attack animation display scaling does not move health bar');
...
assert.equal('knockbackBounceTween' in right,false,'bounce state is removed');
...
combat.damagePlayer=()=>{damageCalls+=1;};
...
const boss=makeEnemy({x:500,isBoss:true}); boss.body.owner=boss; combat.applyKnockback(boss,{knockback:72}); finish(boss); assert.equal(boss.x,509);
...
console.log('[validate-0104-balance-knockback] PASS 0.10.6 combat pacing, stable health bar, and no landing bounce');
