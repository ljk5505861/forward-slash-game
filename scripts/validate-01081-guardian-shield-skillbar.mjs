...
assert.equal(pkg.version,'0.10.81');
...
{ const s=scene(); addLevels(s,1); update(s,5000); update(s,13000); assert.equal(guardians(s).length,0); assert.equal(slotText(s),'稀有 守护盾\nLv.1　充能 5s'); update(s,18000); assert.equal(normal(s).length,1); }
...
{ const s=scene(); addLevels(s,1); const runtime=s.guardianShieldRuntime; assert(runtime); const updaters=s.skillSystem.passiveUpdaters.length, listeners=s.eventBus.count(CombatEvents.PLAYER_DAMAGED); s.playerData.skills=[]; s.skillSystem.removeSkillRuntime('guardian_shield'); assert.equal(s.guardianShieldRuntime,null); assert.equal(state(s),null); s.skillSystem.removeSkillRuntime('guardian_shield'); assert.equal(s.skillSystem.passiveUpdaters.length,updaters-1); assert.equal(s.eventBus.count(CombatEvents.PLAYER_DAMAGED),listeners-1); addLevels(s,1); assert(s.guardianShieldRuntime); assert.notEqual(s.guardianShieldRuntime,runtime); assert.equal(s.skillSystem.passiveUpdaters.length,updaters); assert.equal(s.eventBus.count(CombatEvents.PLAYER_DAMAGED),listeners); const old=s.guardianShieldRuntime; s.skillSystem.reset(); assert.equal(s.guardianShieldRuntime,null); addLevels(s,1); assert(s.guardianShieldRuntime); assert.notEqual(s.guardianShieldRuntime,old); }
...
console.log('v0.10.81 guardian shield skillbar validation passed.');
