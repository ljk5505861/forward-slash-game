...
Object.entries(damageChecks).forEach(([id,fields])=>{
  Object.entries(fields).forEach(([field,expected])=>eq(nums(id,field),expected,`${id}.${field}`));
});
...
const pureOrBuffOnly=[
  'wildfire','eternal_flame_heart','giant_force','spinning_blade','bloodthirst','frenzy','blood_rage_burst','last_stand',
  'healing','guardian_shield','immovable_mountain','black_tortoise_body','shadow_fist','phantom_step','swift_shadow','myriad_afterimage',
];
...
// Non-damage values must remain unchanged.
