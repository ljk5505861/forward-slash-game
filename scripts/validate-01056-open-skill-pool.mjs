...
assert.equal(GAME_VERSION,'0.10.56');
...
try{
  ...
  for(let i=0;i<4000;i+=1){
    ...
    assert.equal(first.rarity==='MYTHIC'||first.ultimateSkill,true,false);
    assert.equal(second.rarity==='MYTHIC'||second.ultimateSkill,true,false);
    assert.equal(
