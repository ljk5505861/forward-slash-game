...
assert.equal(Object.values(SKILLS).filter(s=>!s.temporary).length,29);
...
{ const s=scene(); s.skillSystem.addOrLevel('healing'); run(s); assert.equal(getEffectiveDefense(s.playerData),6); assert.equal(s.playerData.maxShield,50); for(let i=0;i<8;i++) s.skillSystem.addOrLevel('healing'); run(s); assert.equal(getEffectiveDefense(s.playerData),50); assert.equal(s.playerData.damageReductionBonuses.healing,0.10); s.playerData.skills=s.playerData.skills.filter(x=>x.id!=='healing'); s.skillSystem.removeSkillRuntime('healing'); run(s); assert.equal(getEffectiveDefense(s.playerData),0); assert.equal(s.playerData.shield,0); }
