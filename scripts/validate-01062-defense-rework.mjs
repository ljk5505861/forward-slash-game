...
assert.equal(GAME_VERSION,'0.10.67');
assert.equal(Object.values(SKILLS).filter(s=>!s.temporary).length,23);
...
assert.deepEqual(SKILLS.guardian_shield.levels.map(l=>l.intervalMs),[7000,6800,6500,6200,6000,5700,5400,5100,4800]);
...
{ const s=scene(); s.playerData.defense=10; s.skillSystem.addOrLevel('guardian_shield'); run(s); assert.equal(s.playerData.maxShield,80); s.now=7000; run(s); assert.equal(s.playerData.shield,20); s.statusEffects.update(15001); assert.equal(s.playerData.shield,0); for(let i=0;i<8;i++) s.skillSystem.addOrLevel('guardian_shield'); run(s); assert.equal(s.playerData.maxShield,155); s.now=14000; run(s); assert.equal(s.playerData.shield,58); s.statusEffects.update(s.now+999999); assert.equal(s.playerData.shield,58); s.playerData.skills=s.playerData.skills.filter(x=>x.id!=='guardian_shield'); s.skillSystem.removeSkillRuntime('guardian_shield'); run(s); assert.equal(s.playerData.maxShield,50); assert.equal(s.playerData.shield,50); }
{ const s=scene(); for(let i=0;i<3;i++) s.skillSystem.addOrLevel('guardian_shield'); run(s); s.eventBus.emit(CombatEvents.COMBAT_STARTED,{kind:'wave',group:1,wave:1}); assert(s.playerData.shield>0); const first=s.playerData.shield; s.eventBus.emit(CombatEvents.COMBAT_STARTED,{kind:'wave',group:1,wave:1}); assert.equal(s.playerData.shield,first); }
{ const s=scene(), e=enemy(); for(let i=0;i<9;i++) s.skillSystem.addOrLevel('guardian_shield'); run(s); s.statusEffects.add(StatusEffects.SHIELD,s.playerData,{durationMs:1,persistent:true,expiresNaturally:false,value:100,remainingValue:100,sourceId:'guardian_shield'}); s.combatSystem.damagePlayer(e,200,{source:'enemyMelee',undodgeable:true}); assert.equal(s.playerData.shield,0); s.now=999; run(s); assert.equal(s.playerData.shield,0); s.now=1000; run(s); assert.equal(s.playerData.shield,62); }
