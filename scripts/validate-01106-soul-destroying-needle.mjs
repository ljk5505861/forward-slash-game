...
assert.equal(GAME_VERSION,'0.11.19'); assert.equal(pkg.version,'0.11.19'); assert.equal(Object.keys(SKILLS).length,50);
...
s=scene({enemies:[enemy(100,0,200)]}); step(s,0); assert.equal(s.skillSystem.active[0].needles.length,1);step(s,179);assert.equal(s.combatSystem.hits.length,0);step(s,180);assert.equal(s.combatSystem.hits.length,1);step(s,400);assert.equal(s.combatSystem.hits.length,1);assert(s.playerData.mana<=90.8);assert.equal(s.casts||0,0);
...
console.log('v0.11.8 soul destroying needle validation passed.');
